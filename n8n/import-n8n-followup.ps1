...
$workflowPath=Join-Path $PSScriptRoot 'imobia-followup-engine.json'
$bytes=[IO.File]::ReadAllBytes($workflowPath)
$stamp=[DateTimeOffset]::UtcNow.ToUnixTimeSeconds()
$cfg=Invoke-DockerJson "$base/configs/create" 'Post' @{Name="imobia-followup-engine-$stamp";Data=[Convert]::ToBase64String($bytes);Labels=@{'app'='imobia';'role'='n8n-import'}}
...
$mount=@{ConfigID=$cfg.ID;ConfigName="imobia-followup-engine-$stamp";File=@{Name='/tmp/imobia-followup-engine.json';UID='1000';GID='1000';Mode=292}}
$import=New-N8nCliService 'imobia-n8n-followup-import' @('n8n','import:workflow','--input=/tmp/imobia-followup-engine.json') @($mount)
Wait-Task $import.ID 'import'
$activate=New-N8nCliService 'imobia-n8n-followup-activate' @('n8n','update:workflow','--id=ImoBiaFollowup202609','--active=true')
Wait-Task $activate.ID 'activate'
Write-Host 'n8n_followup_ready'
