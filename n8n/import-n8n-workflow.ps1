...
$workflowPath=Join-Path $PSScriptRoot 'imobia-agent-core.json'
$bytes=[IO.File]::ReadAllBytes($workflowPath)
$stamp=[DateTimeOffset]::UtcNow.ToUnixTimeSeconds()
$cfg=Invoke-DockerJson "$base/configs/create" 'Post' @{Name="imobia-agent-core-$stamp";Data=[Convert]::ToBase64String($bytes);Labels=@{'app'='imobia';'role'='n8n-import'}}
...
$mount=@{ConfigID=$cfg.ID;ConfigName="imobia-agent-core-$stamp";File=@{Name='/tmp/imobia-agent-core.json';UID='1000';GID='1000';Mode=292}}
$import=New-N8nCliService 'imobia-n8n-import' @('n8n','import:workflow','--input=/tmp/imobia-agent-core.json') @($mount)
Wait-Task $import.ID 'import'
$activate=New-N8nCliService 'imobia-n8n-activate' @('n8n','update:workflow','--id=ImoBiaCore202609','--active=true')
Wait-Task $activate.ID 'activate'
Write-Host 'n8n_workflow_ready'
