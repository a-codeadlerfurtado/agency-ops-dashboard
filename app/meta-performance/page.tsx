...
const STATUS: Record<string, string> = {
  OK: "OK", PARTIAL_PERIOD: "Período parcial", NO_META_ACCOUNT: "Sem conta Meta", NO_DELIVERY: "Sem entrega",
...
function statusTone(value: unknown) {
  ...
  if (["PARTIAL_PERIOD", "API_PARTIAL", "NO_DELIVERY", "RUNNING", "COMPLETED_WITH_ERRORS"].includes(s)) return "warn";
  return "ok";
}
...
export default function MetaPerformancePage() {
  ...
  useEffect(() => {
    ...
    load(run).then(() => { const client = params.get("client"); if (client) openClient(client, run); });
  ...
  function selectRun(id: string) {
    ...
    setClientId(""); setClientDetail(null); load(id);
  }
  async function download(mode: "summary" | "detailed", client?: string) {
    if (!headers || !payload.selected_run?.id) return;
    const params = new URLSearchParams({ export: mode, run_id: String(payload.selected_run.id) }); if (client) params.set("client_id", client);
    const response = await fetch(`${API}?${params}`, { headers, cache: "no-store" });
  ...
  const status7Groups = Object.entries(STATUS).map(([key,label]) => ({ key, label, rows: clients.filter(c => c.windows[7]?.data_status === key) })).filter(g => g.rows.length);
  ...
  return <main className="mp-shell"><style>{styles}</style>
    ...
    {(clientId || clientLoading) && <><div className="mp-overlay" onClick={closeClient}/><aside className="mp-drawer"><div className="mp-drawer-head"><button onClick={closeClient}>← Voltar</button><b>Histórico Meta do cliente</b></div>{clientLoading&&!clientDetail?<div className="mp-empty">Carregando histórico…</div>:clientDetail&&<ClientHistory detail={clientDetail} download={download} selectedRunId={String(selectedRun?.id||"")} openRun={(runId)=>openClient(clientId,runId)}/>}</aside></>}
  </main>;
}

function ClientHistory({detail,download,selectedRunId,openRun}:{detail:Row;download:(mode:"summary"|"detailed",client?:string)=>Promise<void>;selectedRunId:string;openRun:(id:string)=>void}) {
