...
const STATUS: Record<string, string> = {
  OK: "OK",
  PARTIAL_PERIOD: "Período parcial",
  META_ACCESS_PENDING: "Acesso Meta pendente",
  NO_META_ACCOUNT: "Sem vínculo Meta",
  NO_CAMPAIGNS: "Sem campanha",
  NO_DELIVERY: "Sem entrega",
  API_ERROR: "Erro na API",
  API_PARTIAL: "API parcial",
  PENDING: "Pendente",
  RUNNING: "Processando",
  COMPLETED: "Concluído",
...
function statusTone(value: unknown) {
  ...
  if (["META_ACCESS_PENDING", "PARTIAL_PERIOD", "API_PARTIAL", "NO_CAMPAIGNS", "NO_DELIVERY", "RUNNING", "COMPLETED_WITH_ERRORS"].includes(s)) return "warn";
  return "ok";
}
function statusHint(value: unknown) {
  const s = String(value || "");
  if (s === "META_ACCESS_PENDING") return "A conta é conhecida, mas ainda não está legível pelo acesso da LAK. Revisar compartilhamento do ativo/permissão.";
  if (s === "NO_META_ACCOUNT") return "Ainda não há uma conta de anúncios reconciliada com segurança para este cliente.";
...
export default function MetaPerformancePage() {
  ...
  const visible = useMemo(() => {
    ...
    const statusRank = (c: Row) => {
      ...
      if (["META_ACCESS_PENDING","API_PARTIAL","NO_CAMPAIGNS","NO_DELIVERY","PARTIAL_PERIOD"].includes(s)) return 1;
      if (s === "OK") return 2;
      return 3;
    };
  ...
  const dataStatuses = ["OK","PARTIAL_PERIOD","META_ACCESS_PENDING","NO_META_ACCOUNT","NO_CAMPAIGNS","NO_DELIVERY","API_PARTIAL","API_ERROR"];
  ...
  const accessPendingAll = clients.filter(c=>String(c.windows[7]?.data_status||"")==="META_ACCESS_PENDING").length;
  const noLinkAll = clients.filter(c=>String(c.windows[7]?.data_status||"")==="NO_META_ACCOUNT").length;
  const integratedAll = Math.max(0, clients.length-accessPendingAll-noLinkAll);
  const healthyAll = clients.filter(c=>String(c.windows[7]?.data_status||"")==="OK").length;
  const apiIssuesAll = clients.filter(c=>["API_ERROR","API_PARTIAL"].includes(String(c.windows[7]?.data_status||""))).length;
  ...
  return <main className="mp-shell"><style>{styles}</style>
    ...
    <section className="mp-kpis">
      <article><small>CLIENTES DE TRÁFEGO</small><b>{num(clients.length)}</b><span>escopo deste snapshot</span></article>
      <article><small>COM ACESSO META</small><b>{num(integratedAll)}</b><span>conta efetivamente legível</span></article>
      <article><small>7D SAUDÁVEL</small><b>{num(healthyAll)}</b><span>coleta íntegra</span></article>
      ...
      <article className={(accessPendingAll+noLinkAll)?"warn":""}><small>ACESSO / VÍNCULO META</small><b>{num(accessPendingAll+noLinkAll)}</b><span>{accessPendingAll} acesso pendente · {noLinkAll} sem vínculo</span></article>
      <article className={apiIssuesAll?"bad":""}><small>PROBLEMAS API</small><b>{num(apiIssuesAll)}</b><span>na janela de 7 dias</span></article>
    </section>

    {accessPendingAll>0 && <section className="mp-access-note"><div><b>{accessPendingAll} conta(s) Meta já identificada(s), mas sem leitura pelo acesso da LAK.</b><span>Isso não significa que o cliente não tenha Meta. Significa que o ID é conhecido, porém o ativo/permissão ainda não está disponível ao system user usado pelo coletor.</span></div><button onClick={()=>{setStatus("META_ACCESS_PENDING");setTab("overview");}}>Ver somente esses clientes</button></section>}
    ...
    {tab === "history" && <section className="mp-card"><div className="mp-section-head"><div><h2>Histórico de snapshots</h2><p>Cada quinta-feira fica congelada. Abrir uma data não recalcula o passado.</p></div></div><div className="mp-run-list">{runs.map(run=><button key={run.id} className={String(run.id)===String(selectedRun?.id)?"active":""} onClick={()=>selectRun(String(run.id))}><span><b>{date(run.snapshot_date)}</b><small>dados até {date(run.window_end)}</small></span><span><b>{run.processed_clients}/{run.total_clients}</b><small>{statusLabel(run.status)}</small></span><span><b>{run.metadata?.meta_access_pending_clients || 0}</b><small>acesso Meta pendente</small></span><span><b>{run.no_meta_clients || 0}</b><small>sem vínculo Meta</small></span><span><b>{run.error_clients || 0}</b><small>erros</small></span></button>)}{!runs.length&&<div className="mp-empty">Nenhum snapshot semanal ainda.</div>}</div></section>}

    {tab === "coverage" && <section className="mp-card"><div className="mp-section-head"><div><h2>Cobertura e integrações</h2><p>Somente clientes no escopo de tráfego entram neste relatório. Conta conhecida sem permissão da LAK aparece separada de cliente sem vínculo Meta.</p></div></div><div className="mp-status-grid">{status7Groups.map(group=><article key={group.key} className={statusTone(group.key)}><div><b>{group.label}</b><strong>{group.rows.length}</strong></div>{group.rows.slice(0,40).map(c=><button key={c.client_id} onClick={()=>openClient(c.client_id,selectedRun?.id)}>{c.client_name}<span>{c.gt_owner || "sem GT"}</span></button>)}</article>)}</div></section>}
