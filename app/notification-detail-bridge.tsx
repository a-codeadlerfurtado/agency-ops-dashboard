...
export default function NotificationDetailBridge() {
  ...
  const clientHref = selected.client_id
    ...
    : "";
  const isBriefingNotification = String(selected.source || "").toLocaleLowerCase("pt-BR") === "briefing_hub"
    || String(selected.type || "").toUpperCase().startsWith("BRIEFING_");
  const briefingPath = String(selected.metadata?.dashboard_path || selected.metadata?.deep_link || "").trim();
  const briefingDestination = isBriefingNotification && briefingPath
    ? (/^https?:\/\//i.test(briefingPath)
      ? briefingPath
      : `https://agency-briefing-hub.lakassessoriadigital.workers.dev${briefingPath.startsWith("/") ? "" : "/"}${briefingPath}`)
    : "";
  const briefingHref = briefingDestination && session?.access_token
    ? `${briefingDestination}${briefingDestination.includes("#") ? "&" : "#"}access_token=${encodeURIComponent(session.access_token)}`
    : briefingDestination;
  const externalLinks = [
      ...
      <section className="ndb-card">
        ...
        {loading ? <div className="ndb-loading"><span /> Carregando contexto completo…</div> : <>
          ...
          <div className="ndb-actions">
            {briefingHref && <a className="ndb-primary-action" href={briefingHref}>Abrir briefing preenchido →</a>}
