...
const bucketHelp: Record<string, string> = {
  ...
  SCHEDULED: "Reunião de integração marcada. Dia, hora e link do Meet aparecem no card quando identificados.",
  IN_PROGRESS: "Integração iniciada e ainda em andamento. O link do Meet fica disponível enquanto for o link atual da reunião.",
...
function validMeetUrl(value: unknown) {
  return /^https:\/\/meet\.google\.com\/[A-Za-z0-9-]+(?:\?\S+)?$/i.test(String(value || ""));
}
...
export default function OnboardingPage() {
  ...
  return <main className="ob-shell"><style>{styles}</style>
    ...
    <section className="ob-source"><b>O que entra aqui</b><span>Agendamento, tentativas de reunião, objetivo atingido ou não, motivo de falha, reagendamento, validação de acessos/contas, bloqueios e link do Google Meet quando identificado. O histórico de uma tentativa não é apagado quando outra é marcada.</span></section>
    ...
    <section className="ob-board">
      {groups.map(([bucket, items]) => <article className={`ob-lane ${bucket === "BLOCKED" ? "danger" : bucket === "WAITING_RESCHEDULING" ? "retry" : ""}`} key={bucket}>
        ...
        <div className="ob-cards">{items.map((row) => {
          ...
          const hasMeet = validMeetUrl(row.integration_meet_url);
          return <div className="ob-card" key={String(row.client_id)}>
            ...
            {hasMeet && <div className="ob-meet-wrap"><a className="ob-meet-button" href={String(row.integration_meet_url)} target="_blank" rel="noreferrer">↗ Entrar no Meet</a><small>Link identificado {row.integration_meet_detected_at ? fmtDateTime(row.integration_meet_detected_at) : "automaticamente"}</small></div>}
