...
export default function CommercialClientsPage(){
  ...
  return <main className="shell" style={{paddingLeft:0}}>
    <header className="top" style={{position:"sticky",top:0,zIndex:40}}>
      <div className="brand"><div className="logo"><BrandMark/></div><div><span className="brand-name">Leonardo Imobi</span><h1>Carteira de Clientes</h1><div className="subtitle">Mesma leitura de carteira disponível no perfil de gestão.</div></div></div>
      <div className="live"><a className="btn" href="/commercial-direction">Direção Comercial</a><a className="btn" href="/">Dashboard</a></div>
    </header>
    <div className="source-banner"><span>Carteira executiva.</span> Métricas, retenção, histórico e clientes usam a mesma base da aba Clientes do perfil de gestão.</div>
    {error&&<div className="error-box" style={{margin:20}}>{error}</div>}
    {loading&&!payload.portfolio?<div className="auth-loading"><span className="dot loading"/> Carregando carteira…</div>:<>
