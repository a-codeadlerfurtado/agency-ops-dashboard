...
function HomeView({ data }: { data: Row }) {
  ...
  return <div className="lc-stack">
    <section className="lc-kpis">
      <Kpi label="Leads abertos" value={number(s.open_leads)} hint={`${number(s.new_7d)} novos nos últimos 7 dias`} />
      ...
      <Kpi label="Clientes ativos" value={number(s.active_clients)} hint="ativos + onboarding" />
      <Kpi label="Campanhas ativas" value={number(s.active_campaigns)} hint={`${number(s.meetings_7d)} reuniões em 7 dias`} />
    </section>
