...
const targetingLabel=(row:Row)=>String(row?.metadata?.targeting_label||"").trim();
const targetingApprox=(row:Row)=>Boolean(row?.metadata?.targeting_approx);
    ...
    <section className="ci3">
      <header className="ci3-top">
        <div>
          ...
          <p className="ci3-sub">Catálogo, ranking das peças, ranking dos designers, praça real de segmentação no Meta e performance dos últimos 7 dias. Tudo limitado ao trabalho feito depois da entrada de cada cliente na agência.</p>
        </div>
        <div className="ci3-stats">
          ...
          <Stat value={`${s.targeting_coverage||0}/${s.catalog_total||0}`} label="com praça"/>
      ...
      <div className="ci3-locationbar">
        <div><strong>📍 Praça Meta {location?`· ${location}`:"· todas as regiões"}</strong><p>Vem da segmentação geográfica real do conjunto. Quando o Meta usa somente coordenada/raio, o município aparece como aproximado.</p></div>
        <select value={location} onChange={e=>setLocation(e.target.value)}><option value="">Todas as praças</option>{(payload.filters?.locations||[]).map((x:string)=><option key={x} value={x}>{x}</option>)}</select>
      </div>
      ...
      {tab==="catalog"&&<>
        ...
        {loading?<div className="ci3-loading">Cruzando catálogo Meta, praça e ClickUp…</div>:rows.length?<div className="ci3-grid">{rows.map(r=><CreativeCard key={`${r.client_id}:${r.ad_id}`} row={r} onClick={()=>setDetail(r)}/>)}</div>:<div className="ci3-empty">Nenhum criativo encontrado com esses filtros.</div>}
...
function LocationLine({row}:{row:Row}){
  const place=targetingLabel(row);
  return place?<div className="ci3-location"><span>📍</span><span><b>Praça Meta:</b> {place}</span></div>:<div className="ci3-location missing"><span>📍</span><span>Praça ainda não capturada</span></div>;
}
...
function CreativeCard({row,onClick}:{row:Row;onClick:()=>void}){
  ...
  return <article className="ci3-card" onClick={onClick}>
    ...
    <div className="ci3-card-body"><div className="ci3-client">{row.client_name}</div><div className="ci3-campaign" title={row.campaign_name}>{row.campaign_name||"Campanha sem nome"}</div><LocationLine row={row}/><div className="ci3-title">{row.ad_name||row.creative_name||"Criativo sem nome"}</div><div className="ci3-kpis"><div><small>Resultados</small><b>{num(row.results_7d)}</b></div><div><small>CPR</small><b>{money(row.cost_per_result_7d)}</b></div><div><small>Invest.</small><b>{money(row.spend_7d)}</b></div><div><small>CTR</small><b>{pct(row.ctr_7d,2)}</b></div></div><div className="ci3-author"><div><strong className={attr.trusted?"trusted":attr.level==="FORMATO_INCOMPATIVEL"||attr.level==="CONFLITO"?"warn":""}>{attr.designer||"Autoria não confirmada"}</strong><span> · {attr.level||"NAO_IDENTIFICADO"}</span></div><span>{proofLabel(row.performance?.proof)}</span></div></div>
  </article>;
}
...
function CreativeRanking({rows,loading,error,onOpen,location}:{rows:Row[];loading:boolean;error:string;onOpen:(row:Row)=>void;location:string}){
  ...
  return <>
    <div className="ci3-ranking-note"><b>Ranking de criativos{location?` · ${location}`:""}:</b> a peça entra pelo resultado dela, mesmo que a autoria ainda não esteja confirmada. Quando uma praça é selecionada, só entram anúncios cuja segmentação Meta inclui aquela região.</div>
    <div className="ci3-creative-podium">{top.map(r=><article className="ci3-creative-rank" key={`${r.client_id}:${r.ad_id}`} onClick={()=>onOpen(r)}><div className="ci3-creative-rank-media">{r.preview_url||r.image_url?<img src={r.preview_url||r.image_url} alt=""/>:<div className="ci3-img-empty">Prévia indisponível</div>}<span className="ci3-creative-rank-position">#{r.creative_rank}</span><span className="ci3-creative-rank-score">{r.performance?.score||0}/100</span></div><div className="ci3-creative-rank-body"><div className="ci3-client">{r.client_name}</div><h3>{r.ad_name||r.creative_name||"Criativo sem nome"}</h3><p>{r.campaign_name||"Campanha sem nome"}</p><LocationLine row={r}/><span className="ci3-proof">{proofLabel(r.performance?.proof)}</span></div></article>)}</div>
    <div className="ci3-rank-table"><div className="ci3-rank-row ci3-creative-row head"><span>#</span><span>Criativo</span><span>Cliente / praça</span><span>Score</span><span>Resultados</span><span>CPR</span><span>Invest.</span></div>{rows.map(r=><div className="ci3-rank-row ci3-creative-row clickable" key={`rank:${r.client_id}:${r.ad_id}`} onClick={()=>onOpen(r)}><b>#{r.creative_rank}</b><div className="ci3-creative-cell"><strong>{r.ad_name||r.creative_name||"Criativo sem nome"}</strong><small>{proofLabel(r.performance?.proof)} · {r.creative_format||"—"}{r.attribution?.designer?` · ${r.attribution.designer}`:""}</small></div><div className="ci3-creative-cell"><strong>{r.client_name}</strong><small>{r.campaign_name||"Campanha sem nome"}</small><small className="place">📍 {targetingLabel(r)||"Praça não capturada"}</small></div><b>{r.performance?.score||0}</b><span>{num(r.results_7d)}</span><span>{money(r.cost_per_result_7d)}</span><span>{money(r.spend_7d)}</span></div>)}</div>
  </>;
}
