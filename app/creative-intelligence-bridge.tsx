...
const resolvedCity=(row:Row)=>String(row?.metadata?.resolved_location?.display||row?.metadata?.targeting_label||"").trim();
  ...
  return createPortal(<>
    ...
    <section className="ci3">
      ...
      <div className="ci3-locationbar">
        <div><strong>📍 Cidade dos anúncios {location?`· ${location}`:"· todas as cidades"}</strong><p>A cidade é consolidada a partir da segmentação Meta, coordenadas, campanha e briefing. A interface não exibe raio bruto nem praça sem identificação.</p></div>
...
function LocationLine({row}:{row:Row}){
  const place=resolvedCity(row);
  return <div className="ci3-location"><span>📍</span><span><b>Cidade:</b> {place}</span></div>;
}
