...
.ci3-coverage{margin:16px 0 10px;display:flex;justify-content:space-between;align-items:center;gap:14px;padding:13px 15px;border:1px solid rgba(74,183,245,.2);border-radius:13px;background:#0b2030}.ci3-coverage strong{font-size:12px}.ci3-coverage p{margin:3px 0 0;color:#789fb4;font-size:10px}
...
.ci3-locationbar{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:12px 15px;border:1px solid rgba(84,221,153,.18);border-radius:13px;background:rgba(13,48,43,.38)}.ci3-locationbar strong{display:block;font-size:11px}.ci3-locationbar p{margin:3px 0 0;color:#80a99d;font-size:9px}.ci3-locationbar select{min-width:240px;max-width:360px;height:38px;border-radius:10px;border:1px solid rgba(84,221,153,.22);background:#0b2030;color:#eaf5fb;padding:0 10px;font-size:10px;outline:none}
...
.ci3-card-body{padding:12px}.ci3-client{font-size:11px;font-weight:900}.ci3-campaign{font-size:9px;color:#6e99af;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:3px}.ci3-location{display:flex;gap:5px;align-items:flex-start;margin-top:7px;color:#78caa4;font-size:8.5px;line-height:1.35}.ci3-location b{color:#9adfbe;font-weight:850}.ci3-location-copy{min-width:0;flex:1}.ci3-location-list{display:flex;flex-wrap:wrap;gap:4px;margin-top:4px}.ci3-city-chip{display:inline-flex;max-width:100%;padding:3px 6px;border-radius:999px;border:1px solid rgba(84,221,153,.2);background:rgba(84,221,153,.08);color:#a8e5c5;font-size:8px;font-weight:800;line-height:1.15}.ci3-title{font-size:11px;line-height:1.35;margin:8px 0;color:#dcecf4;min-height:29px}.ci3-manager-link{display:inline-flex;align-items:center;gap:5px;margin:0 0 9px;color:#63c7ff;text-decoration:none;font-size:8.5px;font-weight:900}.ci3-manager-link:hover{color:#a8e2ff;text-decoration:underline}.ci3-manager-primary{margin:10px 0 4px;padding:10px 13px;border:1px solid rgba(74,183,245,.35);border-radius:9px;background:#12364d;color:#eef9ff;font-size:10px;text-decoration:none}.ci3-manager-primary:hover{background:#17445f;text-decoration:none;color:#fff}.ci3-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;padding-top:8px;border-top:1px solid rgba(126,180,210,.11)}.ci3-kpis small{display:block;color:#648da4;font-size:7px;text-transform:uppercase}.ci3-kpis b{font-size:10px}.ci3-author{margin-top:9px;padding-top:8px;border-top:1px solid rgba(126,180,210,.1);display:flex;justify-content:space-between;gap:8px;align-items:center}.ci3-author strong{font-size:9px}.ci3-author span{font-size:8px;color:#6d98ad}.ci3-author .trusted{color:#54dd99}.ci3-author .warn{color:#ffad66}
...
const locationCities=(row:Row)=>{const resolved=Array.isArray(row?.metadata?.resolved_location?.cities)?row.metadata.resolved_location.cities:[];const targeting=row?.metadata?.targeting||{};const fallback=[...(Array.isArray(targeting.cities)?targeting.cities:[]),...(Array.isArray(targeting.inferred_cities)?targeting.inferred_cities:[])];return [...new Set([...resolved,...fallback].map(x=>String(x||"").trim()).filter(Boolean))];};
const resolvedCity=(row:Row)=>{const cities=locationCities(row);return cities.length?cities.join(", "):String(row?.metadata?.resolved_location?.display||row?.metadata?.targeting_label||"").trim();};
const targetingPoints=(row:Row)=>Array.isArray(row?.metadata?.targeting?.custom_points)?row.metadata.targeting.custom_points:[];
const locationSourceLabel=(row:Row)=>({META_TARGETING:"Meta · conjunto de anúncios",CAMPAIGN_INHERIT:"Outra configuração da mesma campanha",CLIENT_BRIEFING:"Briefing do cliente",CLIENT_DOMINANT:"Histórico dominante do cliente"} as Row)[String(row?.metadata?.resolved_location?.source||"")]||"Evidência operacional consolidada";
const locationLevelLabel=(row:Row)=>({CITY:"Cidades do Meta",REGION:"Região do Meta",CUSTOM:"Pontos / raio no Meta",ZIP:"CEP no Meta",COUNTRY:"País no Meta",UNKNOWN:"Fallback operacional"} as Row)[String(row?.metadata?.targeting_level||"")]||"Consolidada";
const adsManagerUrl=(row:Row)=>{const act=String(row?.meta_ad_account_id||"").replace(/^act_/i,"").trim(),ad=String(row?.ad_id||"").trim();if(!act||!ad)return"";const p=new URLSearchParams({act,selected_ad_ids:ad});if(row?.campaign_id)p.set("selected_campaign_ids",String(row.campaign_id));if(row?.adset_id)p.set("selected_adset_ids",String(row.adset_id));return`https://adsmanager.facebook.com/adsmanager/manage/ads?${p.toString()}`;};
...
export default function CreativeIntelligenceBridge(){
  ...
  useEffect(()=>{setPage(1);},[query,client,campaign,format,designer,proof,status,location]);
  useEffect(()=>{
  ...
  const requestUrl=useMemo(()=>{
    ...
    return `${API}?${p}`;
  },[page,query,client,campaign,format,designer,proof,status,location]);
  ...
  useEffect(()=>{
    if(!mount||tab!=="creativeRanking")return;
    setRankLoading(true);setRankError("");
    const p=new URLSearchParams({page:"1",page_size:"96"});
    if(location)p.set("location",location);
    authenticatedFetch(`${API}?${p}`,{cache:"no-store"})
  ...
  const clearFilters=()=>{setQuery("");setClient("");setCampaign("");setFormat("");setDesigner("");setProof("");setStatus("");setLocation("");};
    ...
    <section className="ci3">
      <header className="ci3-top">
        <div>
          ...
          <p className="ci3-sub">Catálogo, ranking das peças, ranking dos designers, cidade/região de anúncio e performance dos últimos 7 dias. Tudo limitado ao trabalho feito depois da entrada de cada cliente na agência.</p>
        </div>
        <div className="ci3-stats">
          ...
          <Stat value={`${s.targeting_coverage||0}/${s.catalog_total||0}`} label="com cidade"/>
      ...
      <div className="ci3-locationbar">
        <div><strong>📍 Praça detalhada {location?`· ${location}`:"· todas as cidades"}</strong><p>O card mostra todas as cidades do anúncio. Clique em qualquer criativo para ver também origem da praça, confiança da resolução e os pontos/raios usados no Meta quando existirem.</p></div>
        <select value={location} onChange={e=>setLocation(e.target.value)}><option value="">Todas as cidades</option>{(payload.filters?.locations||[]).map((x:string)=><option key={x} value={x}>{x}</option>)}</select>
      </div>
      ...
      {tab==="catalog"&&<>
        <div className="ci3-filters">
          <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar cliente, campanha, anúncio ou cidade…"/>
        ...
        <div className="ci3-filter-row"><span>{s.filtered_total||0} resultado(s){location?` em ${location}`:""}</span><button onClick={clearFilters}>Limpar filtros</button><select value={status} onChange={e=>setStatus(e.target.value)} style={{marginLeft:"auto",background:"#0b2030",color:"#d8edf7",border:"1px solid rgba(126,180,210,.16)",borderRadius:8,padding:"6px 8px",fontSize:9}}><option value="">Qualquer status</option><option value="ACTIVE">Ativas</option><option value="PAUSED">Pausadas</option></select></div>
        {loading?<div className="ci3-loading">Cruzando catálogo Meta, cidade e ClickUp…</div>:rows.length?<div className="ci3-grid">{rows.map(r=><CreativeCard key={`${r.client_id}:${r.ad_id}`} row={r} onClick={()=>setDetail(r)}/>)}</div>:<div className="ci3-empty">Nenhum criativo encontrado com esses filtros.</div>}
      ...
      {tab==="creativeRanking"&&<CreativeRanking rows={creativeRanking} loading={rankLoading} error={rankError} onOpen={setDetail} location={location}/>} 
      {tab==="designerRanking"&&<DesignerRanking ranking={ranking} location={location}/>} 
...
function LocationLine({row}:{row:Row}){
  const cities=locationCities(row),fallback=resolvedCity(row);
  return <div className="ci3-location"><span>📍</span><div className="ci3-location-copy"><b>{cities.length>1?"Cidades":"Cidade"}:</b><div className="ci3-location-list">{cities.length?cities.map(city=><span className="ci3-city-chip" key={city}>{city}</span>):<span className="ci3-city-chip">{fallback}</span>}</div></div></div>;
}

function CreativeCard({row,onClick}:{row:Row;onClick:()=>void}){
  const active=String(row.campaign_status||row.ad_status||"").includes("ACTIVE"),attr=row.attribution||{},manager=adsManagerUrl(row);
  return <article className="ci3-card" onClick={onClick}>
    ...
    <div className="ci3-card-body"><div className="ci3-client">{row.client_name}</div><div className="ci3-campaign" title={row.campaign_name}>{row.campaign_name||"Campanha sem nome"}</div><LocationLine row={row}/><div className="ci3-title">{row.ad_name||row.creative_name||"Criativo sem nome"}</div>{manager?<a className="ci3-manager-link" href={manager} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()}>Abrir no Gerenciador ↗</a>:null}<div className="ci3-kpis"><div><small>Resultados</small><b>{num(row.results_7d)}</b></div><div><small>CPR</small><b>{money(row.cost_per_result_7d)}</b></div><div><small>Invest.</small><b>{money(row.spend_7d)}</b></div><div><small>CTR</small><b>{pct(row.ctr_7d,2)}</b></div></div><div className="ci3-author"><div><strong className={attr.trusted?"trusted":attr.level==="FORMATO_INCOMPATIVEL"||attr.level==="CONFLITO"?"warn":""}>{attr.designer||"Autoria não confirmada"}</strong><span> · {attr.level||"NAO_IDENTIFICADO"}</span></div><span>{proofLabel(row.performance?.proof)}</span></div></div>
  </article>;
}

function CreativeRanking({rows,loading,error,onOpen,location}:{rows:Row[];loading:boolean;error:string;onOpen:(row:Row)=>void;location:string}){
  if(error)return <div className="ci3-error">{error}</div>;
  if(loading)return <div className="ci3-loading">Calculando os melhores criativos {location?`de ${location}`:"da operação"}…</div>;
  if(!rows.length)return <div className="ci3-empty">Ainda não existe amostra suficiente para ranquear criativos{location?` em ${location}`:""}.</div>;
  const top=rows.slice(0,3);
  return <>
    <div className="ci3-ranking-note"><b>Ranking de criativos{location?` · ${location}`:""}:</b> a peça entra pelo resultado dela, mesmo que a autoria ainda não esteja confirmada. Quando uma cidade é selecionada, só entram anúncios associados àquela cidade/região.</div>
    <div className="ci3-creative-podium">{top.map(r=><article className="ci3-creative-rank" key={`${r.client_id}:${r.ad_id}`} onClick={()=>onOpen(r)}><div className="ci3-creative-rank-media">{r.preview_url||r.image_url?<img src={r.preview_url||r.image_url} alt=""/>:<div className="ci3-img-empty">Prévia indisponível</div>}<span className="ci3-creative-rank-position">#{r.creative_rank}</span><span className="ci3-creative-rank-score">{r.performance?.score||0}/100</span></div><div className="ci3-creative-rank-body"><div className="ci3-client">{r.client_name}</div><h3>{r.ad_name||r.creative_name||"Criativo sem nome"}</h3><p>{r.campaign_name||"Campanha sem nome"}</p><LocationLine row={r}/>{adsManagerUrl(r)?<a className="ci3-manager-link" href={adsManagerUrl(r)} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()}>Abrir no Gerenciador ↗</a>:null}<span className="ci3-proof">{proofLabel(r.performance?.proof)}</span></div></article>)}</div>
    <div className="ci3-rank-table"><div className="ci3-rank-row ci3-creative-row head"><span>#</span><span>Criativo</span><span>Cliente / cidades</span><span>Score</span><span>Resultados</span><span>CPR</span><span>Invest.</span></div>{rows.map(r=><div className="ci3-rank-row ci3-creative-row clickable" key={`rank:${r.client_id}:${r.ad_id}`} onClick={()=>onOpen(r)}><b>#{r.creative_rank}</b><div className="ci3-creative-cell"><strong>{r.ad_name||r.creative_name||"Criativo sem nome"}</strong><small>{proofLabel(r.performance?.proof)} · {r.creative_format||"—"}{r.attribution?.designer?` · ${r.attribution.designer}`:""}</small></div><div className="ci3-creative-cell"><strong>{r.client_name}</strong><small>{r.campaign_name||"Campanha sem nome"}</small><small className="place">📍 {resolvedCity(r)}</small></div><b>{r.performance?.score||0}</b><span>{num(r.results_7d)}</span><span>{money(r.cost_per_result_7d)}</span><span>{money(r.spend_7d)}</span></div>)}</div>
  </>;
}

function DesignerRanking({ranking,location}:{ranking:Row[];location:string}){
  const ranked=ranking.filter(r=>r.pieces>0),top=ranked.slice(0,3);
  return <>
    <div className="ci3-ranking-note"><b>Ranking de designers{location?` · ${location}`:""}:</b> só entram criativos cuja autoria seja <b>FORTE ou CONFIRMADA</b>. {location?`Com a cidade selecionada, a comparação considera somente peças anunciadas em ${location}.`:"O filtro de cidade acima também pode recalcular este ranking por região."}</div>
    {top.length?<div className="ci3-podium">{top.map(r=><RankCard key={r.designer} row={r} top/>)}</div>:<div className="ci3-empty">Ainda não existe amostra de autoria forte suficiente para ranquear designers{location?` em ${location}`:""}.</div>}
...
function Method({payload}:{payload:Row}){return <div className="ci3-method"><article><h3>Ranking de criativos</h3><p>A peça é avaliada pela performance dos últimos 7 dias usando eficiência, volume, CTR, conversão e força da amostra em relação à referência do próprio cliente. <b>Não precisa ter autoria confirmada</b> para aparecer nesse ranking.</p></article><article><h3>Praça detalhada</h3><p>Todos os municípios associados ao anúncio aparecem individualmente, sem resumo do tipo “+2 cidades”. No detalhe da peça também mostramos a origem da resolução e, quando a segmentação do Meta usa pontos/raios, cada ponto original com seu município e raio correspondente.</p></article><article><h3>Ranking de designers</h3><p>{payload.methodology?.ranking||""} Aqui a autoria precisa ser FORTE ou CONFIRMADA, porque o objetivo é medir o trabalho da pessoa e não somente a peça.</p></article><article><h3>Autoria: mais rígida</h3><p>{payload.methodology?.attribution||""} Tasks chamadas <b>Campanha</b> ou <b>Ajuste na campanha</b> não dão autoria ao designer.</p></article><article><h3>Corte pós-entrada</h3><p>{payload.methodology?.catalog||""} Isso impede que criativos antigos do cliente entrem no catálogo ou em qualquer ranking como se fossem produção da agência.</p></article></div>;}

function Detail({row,onClose}:{row:Row;onClose:()=>void}){
  const attr=row.attribution||{},cities=locationCities(row),manager=adsManagerUrl(row),points=targetingPoints(row),resolved=row?.metadata?.resolved_location||{},confidence=finite(resolved?.confidence),regions=Array.isArray(row?.metadata?.targeting?.regions)?row.metadata.targeting.regions:[];
  return <div className="ci3-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><aside className="ci3-drawer"><button className="ci3-close" onClick={onClose}>×</button><small>{row.client_name} · {row.creative_format}</small><h2>{row.ad_name||row.creative_name||"Criativo"}</h2><p className="ci3-drawer-sub">{row.campaign_name} · {row.adset_name||"Conjunto não identificado"}</p>{manager?<a className="ci3-manager-link ci3-manager-primary" href={manager} target="_blank" rel="noreferrer">Abrir anúncio no Gerenciador ↗</a>:null}{row.preview_url||row.image_url?<img className="ci3-preview" src={row.preview_url||row.image_url} alt=""/>:null}<div className="ci3-detail-grid"><div><small>Resultados 7d</small><b>{num(row.results_7d)}</b></div><div><small>CPR 7d</small><b>{money(row.cost_per_result_7d)}</b></div><div><small>Investimento</small><b>{money(row.spend_7d)}</b></div><div><small>CTR</small><b>{pct(row.ctr_7d,2)}</b></div></div><div className="ci3-panel"><small>Praça detalhada</small><h3>📍 {cities.length===1?cities[0]:`${cities.length} cidades`}</h3><div className="ci3-location-list">{cities.map(city=><span className="ci3-city-chip" key={city}>{city}</span>)}</div><div className="ci3-place-meta"><div><small>Origem</small><b>{locationSourceLabel(row)}</b></div><div><small>Confiança</small><b>{confidence===null?"—":`${Math.round(confidence)}%`}</b></div><div><small>Tipo de praça</small><b>{locationLevelLabel(row)}</b></div></div>{regions.length?<p><b>Região no Meta:</b> {regions.join(", ")}</p>:null}{points.length?<div className="ci3-radius-list"><p><b>Segmentação original por ponto/raio no Meta:</b></p>{points.map((point:Row,i:number)=><div className="ci3-radius-row" key={`${point.latitude||"x"}:${point.longitude||i}`}><b>{point.city||cities[i]||"Ponto geográfico"}</b>{point.radius!==null&&point.radius!==undefined?` · raio ${point.radius} ${String(point.distance_unit||"").toLowerCase().includes("mile")?"mi":"km"}`:""}{point.approx?" · município resolvido pela coordenada":""}</div>)}</div>:<p>Esta praça não depende de raio geográfico bruto; as cidades acima são a referência consolidada do anúncio.</p>}</div><div className="ci3-panel"><small>Autoria</small><h3>{attr.designer||"Não confirmada"} · {attr.level||"NAO_IDENTIFICADO"}</h3><p>Confiança: {attr.confidence||0}% · Método: {attr.method||"sem evidência suficiente"}</p>{(attr.warnings||[]).map((w:string,i:number)=><p className="ci3-warning" key={i}>⚠ {w}</p>)}{(attr.evidences||[]).map((e:Row,i:number)=><div className="ci3-evidence" key={i}><b>{e.source}</b> · {e.label}{e.url?<><br/><a href={e.url} target="_blank" rel="noreferrer">Abrir evidência</a></>:null}</div>)}</div><div className="ci3-panel"><small>Performance</small><h3>{proofLabel(row.performance?.proof)} · score {row.performance?.score||0}/100</h3>{(row.performance?.reasons||[]).map((x:string,i:number)=><p key={i}>{x}</p>)}</div><div className="ci3-panel"><small>Meta</small><p>Campanha: {row.campaign_status||"—"} · Anúncio: {row.ad_status||"—"}</p><p>ID do anúncio: {row.ad_id} · ID do criativo: {row.creative_id||"—"}</p>{manager?<p><a className="ci3-manager-link" href={manager} target="_blank" rel="noreferrer">Ver este anúncio na íntegra ↗</a></p>:null}</div></aside></div>;
}
