...
function removeCommercialNavigation() {
  document.querySelectorAll("[data-commercial-funnel-nav],[data-commercial-section-nav],[data-commercial-director-nav],[data-commercial-director-item]")
...
export default function CommercialNavigationBridge() {
  ...
  // COMMERCIAL usa o próprio shell do dashboard. Nenhuma rota, botão ou listener
  // desta ponte pode sequestrar Clientes/Home e mandar Leonardo para outra página.
  useEffect(() => {
  ...
  useEffect(() => {
    if (profileRole === "COMMERCIAL" || session?.user?.id === GABRIEL_USER_ID || !session?.access_token) {
      setFridayAllowed(false);
      return;
    }
