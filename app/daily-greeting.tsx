...
const FULL_GREETING_AUDIO = "/audio/opsquestion-greeting-full-v6.mp3?v=20260822-full-v6";
const FULL_GREETING_SECONDS = 22.804898;
...
export default function DailyGreeting() {
  ...
  const hasOpeningAudio = Boolean(
    greeting && (greeting.is_monday || greeting.test_mode === "ADLER_EVERY_LOGIN" || greeting.audio_duration_seconds || greeting.audio_url || greeting.audio_parts?.length),
  );
  ...
  useEffect(() => {
    ...
    async function claim(session: Session | null) {
      ...
      try {
        ...
        // O bom dia é complementar e nunca pode derrubar o dashboard.
      }
    }
  ...
  useEffect(() => {
    ...
    async function prepare() {
      try {
        // Baixa o MP3 inteiro antes de criar o player. Evita streaming parcial e cache quebrado.
        const response = await fetch(FULL_GREETING_AUDIO, {
          cache: "reload",
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          const fail = () => { cleanup(); reject(new Error("audio_decode_failed")); };
          const cleanup = () => {
  ...
  }, [greeting?.date, greeting?.test_mode, hasOpeningAudio]);
  ...
  return (
    <div role="presentation" style={{ position: "fixed", inset: 0, zIndex: 30000, background: "radial-gradient(circle at 50% 35%, rgba(22,91,132,.20), rgba(1,8,14,.93) 48%, rgba(1,6,11,.98) 100%)", backdropFilter: "blur(9px)", display: "grid", placeItems: "center", padding: 18 }}>
      <style>{`
        @keyframes opsqOrbSpeak { 0%,100% { transform: scale(1); filter: brightness(1); } 50% { transform: scale(1.055); filter: brightness(1.16); } }
        @keyframes opsqWaveOut { 0% { transform: translate(-50%,-50%) scale(.72); opacity: .72; } 100% { transform: translate(-50%,-50%) scale(1.5); opacity: 0; } }
        @keyframes opsqBarSpeak { 0%,100% { transform: scaleY(.32); opacity: .35; } 50% { transform: scaleY(1); opacity: 1; } }
        @keyframes opsqNetworkPulse { 0%,100% { opacity: .58; } 50% { opacity: 1; } }
      ...
      <section role="dialog" aria-modal="true" aria-label="Bom dia do OpsQuestion" style={{ width: "min(640px, 100%)", border: monday ? "1px solid rgba(79,190,255,.48)" : "1px solid rgba(82,153,198,.35)", background: "linear-gradient(180deg, rgba(6,25,39,.99), rgba(3,14,24,.99))", borderRadius: 20, boxShadow: monday ? "0 30px 110px rgba(0,0,0,.72), 0 0 54px rgba(49,151,213,.13)" : "0 30px 100px rgba(0,0,0,.68)", color: "#eef8ff", padding: "24px 28px", display: "grid", gap: 15, overflow: "hidden", position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <span style={{ color: "#72bff0", fontSize: 10.5, fontWeight: 900, letterSpacing: ".14em" }}>{monday ? "OPSQUESTION · INÍCIO DE SEMANA" : "OPSQUESTION · ONLINE"}</span>
        ...
        <div style={{ display: "grid", gap: 8, textAlign: "center" }}>
          ...
          <p style={{ margin: "0 auto", color: "#adc4d4", fontSize: 14.5, lineHeight: 1.6, maxWidth: 520 }}>{monday ? "Nova semana operacional iniciada. Organização, execução e bons resultados por aí." : "OpsQuestion online. Que tenhamos um dia produtivo e uma excelente execução nas atividades de hoje."}</p>
        </div>
          ...
          <div style={{ border: "1px solid rgba(82,172,225,.18)", background: "rgba(15,51,73,.42)", borderRadius: 12, padding: "11px 13px", display: "grid", gap: 9 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              ...
              {!audioFinished && !audioLoading && audioBlocked && (
                <button type="button" onClick={playOpening} autoFocus style={{ border: "1px solid rgba(94,187,242,.45)", background: "#0b314a", color: "#e8f7ff", borderRadius: 9, padding: "8px 11px", cursor: "pointer", font: "inherit", fontSize: 10.5, fontWeight: 850 }}>▶ Tocar abertura</button>
              )}
            </div>
