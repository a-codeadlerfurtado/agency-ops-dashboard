...
type FieldOption = { kind: "required" | "extra"; label: string; value: string };
...
export default function LeadQualityAlert() {
  ...
  const fieldOptions = useMemo<FieldOption[]>(() => {
    ...
    return [
      ...required.map((label) => ({ kind: "required" as const, label, value: `required:${label}` })),
      ...extras.map((item, index) => {
        const label = item.question || `campo adicional ${index + 1}`;
        return { kind: "extra" as const, label, value: `extra:${label}` };
      }),
    ];
  }, [incident]);

  useEffect(() => {
    setFieldSelection(fieldOptions[0]?.value ?? "");
  }, [incident?.id, fieldOptions]);
  ...
  const acknowledge = useCallback(async () => {
    ...
    try {
      const response = await fetch(API_URL, {
        ...
        body: JSON.stringify({ incident_id: incident.id, action: "acknowledge" }),
      });
      if (!response.ok) return;
      await load();
    } finally {
  ...
  const suppressField = useCallback(async () => {
    if (!incident || !session?.access_token || suppressing || !fieldSelection) return;
    const selected = fieldOptions.find((item) => item.value === fieldSelection);
    if (!selected) return;
    setSuppressing(true);
    try {
      const response = await fetch(API_URL, {
        ...
        body: JSON.stringify({
          ...
          action: "suppress_field",
          field_kind: selected.kind,
  ...
  }, [fieldOptions, fieldSelection, incident, load, session?.access_token, suppressing]);
      ...
      <section className="lqa-card">
        ...
        <div className="lqa-missing">
          ...
          {fieldOptions.length > 0 && (
            <div className="lqa-exemption">
              <div>
                <b>Campo retirado a pedido do cliente?</b>
                <span>Selecione o campo. Ele deixa de gerar alerta para este cliente + produto.</span>
              </div>
              <div className="lqa-exemption-actions">
                <select value={fieldSelection} onChange={(event) => setFieldSelection(event.target.value)} disabled={suppressing} aria-label="Campo retirado a pedido do cliente">
                  {fieldOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
                </select>
                <button type="button" disabled={suppressing || !fieldSelection} onClick={suppressField}>{suppressing ? "Salvando…" : "Não alertar mais"}</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="lqa-footer">
          ...
          <button disabled={acking || suppressing} onClick={acknowledge}>{acking ? "Registrando…" : "Ciente — vou resolver"}</button>
        </div>
      </section>
    </div>
  );
