class Whatsapp::FacebookApiClient
  ...
  def exchange_code_for_token(code)
    return exchange_code_via_broker(code) if broker_configured?
  ...
  def exchange_code_via_broker(code)
    response = HTTParty.post(
      ENV.fetch('META_BROKER_URL'),
      headers: {
        'Authorization' => "Bearer #{ENV.fetch('META_BROKER_TOKEN')}",
        'Content-Type' => 'application/json'
      },
      body: { code: code }.to_json
    )
    handle_response(response, 'Broker token exchange failed')
  end
