...
export default function OnboardingCommandBar() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setBusy(true);
    setFeedback({ tone: "info", text: "Interpretando, auditando e aplicando a alteração…" });
    try {
  ...
  return <aside className={`ocb ${collapsed ? "collapsed" : ""}`}>
    ...
    <div className="ocb-head">
      <div><span>COMANDO ADMINISTRATIVO · ADLER</span><b>Alterar qualquer estado do onboarding</b></div>
    ...
    {!collapsed && <>
      <form onSubmit={submit}>
        <input
          ...
          placeholder="Ex.: Estela reagenda integração para segunda 17h · muda GT para Yuri · bloqueia aguardando cliente"
          disabled={busy}
        />
        <button type="submit" disabled={busy || !command.trim()}>{busy ? "Aplicando…" : "Aplicar alteração"}</button>
      </form>
      <p>Você pode concluir, reabrir, voltar ou reagendar etapas; bloquear/desbloquear; mudar GT, CS ou designer; alterar risco, próxima ação, prazos, notas e status. Toda mudança grava antes/depois. Para edição ultraespecífica: <code>case.next_action=...</code>, <code>client.gt_owner=...</code> ou <code>stage[INTEGRATION_MEETING].status=SCHEDULED</code>.</p>
