...
function groupByClient(items: WorkItem[]) {
  const grouped = new Map<string, WorkItem[]>();

  for (const item of items) {
    const bucket = grouped.get(item.client_id) ?? [];
    bucket.push(item);
    grouped.set(item.client_id, bucket);
  }

  return [...grouped.entries()]
    .map(([clientId, clientItems]) => ({
      clientId,
      displayName: clientItems[0]?.display_name ?? "Cliente",
      items: clientItems,
    }))
...
export function AIWorkBoard({ token }: { token: string }) {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      setSelected((current) => current
        ? (next.items || []).find((item: WorkItem) => item.id === current.id) || current
        : null);
    } catch (caught) {
  ...
  const visible = useMemo(() => items.filter((item) => {
    ...
    return !needle
      || `${item.display_name} ${item.title} ${item.description || ""}`
  ...
  function itemsFor(column: string) {
    if (column === "WAITING") {
      return visible.filter((item) => ["WAITING", "SNOOZED"].includes(item.status));
    }
  ...
  async function completeSelected() {
    ...
    await updateItem(selected, {
      status: "COMPLETED",
      resolution: completion.trim(),
    });
    setCompletion("");
  }
  ...
  return <section className="aiw-wrap">
    <div className="aiw-head">
      <div>
        <span className="aih-kicker">CENTRAL DE TRABALHO · IA</span>
        <h2>Quadro do Castro</h2>
        <p>
          Só entram demandas dos clientes com nossa IA ativa ou em implantação.
          O n8n indica o estágio técnico; tráfego, design, CS e comercial continuam fora deste quadro.
        </p>
      </div>
      <div className="aiw-head-actions">
        <button onClick={() => load()} disabled={loading}>Atualizar</button>
        <button className="aiw-primary" onClick={() => setCreateOpen((open) => !open)}>
          {createOpen ? "Cancelar" : "+ Nova demanda"}
        </button>
      </div>
    </div>
    ...
    <div className="aiw-kpis">
      ...
      <article className={summary.overdue ? "bad" : ""}>
        <small>ATRASADAS</small><b>{summary.overdue}</b>
      </article>
    </div>
    ...
    {createOpen && <form className="aiw-create" onSubmit={createItem}>
        ...
        <label>
          ...
          >
            ...
            {clients.map((client) => <option key={client.id} value={client.id}>
              {client.display_name} · {client.service?.service_status === "BUILDING" ? "implantação" : "ativa"}
            </option>)}
          </select>
        </label>
      ...
      <label>
        ...
        <input
          ...
          placeholder="Ex.: corrigir retorno do agente no WhatsApp"
        />
      </label>
      ...
      <label>
        ...
        <textarea
          ...
          placeholder="O que aconteceu, evidência e resultado esperado"
        />
      </label>

      <div className="aiw-create-foot">
        <span>Responsável: Gabriel Castro · Head de IA</span>
        <button className="aiw-primary" disabled={busy === "create"}>
          {busy === "create" ? "Criando…" : "Criar demanda"}
        </button>
      </div>
    </form>}
    ...
    <div className="aiw-toolbar">
      ...
      <select value={clientFilter} onChange={(event) => setClientFilter(event.target.value)}>
        <option value="ALL">Todos os clientes IA</option>
        {clients.map((client) => <option key={client.id} value={client.id}>
          {client.display_name}
        </option>)}
      </select>
    ...
    {loading && !payload
      ? <div className="aih-empty">Carregando quadro de trabalho…</div>
      : <div className="aiw-board">
        {COLUMNS.map((column) => {
          const columnItems = itemsFor(column.key);
          const groups = groupByClient(columnItems);

          return <section
            className={`aiw-column aiw-column-${column.key.toLowerCase()}`}
            key={column.key}
            onDragOver={(event) => {
              if (column.key !== "COMPLETED") event.preventDefault();
            }}
            onDrop={(event) => {
              event.preventDefault();
          ...
          >
            ...
            <div className="aiw-cards">
              {groups.map((group) => {
                ...
                const stage = client?.service?.service_status === "BUILDING"
                  ? "EM IMPLANTAÇÃO"
                  : client?.n8n_verified
                    ? "N8N ATIVO"
                    : "IA ATIVA";
                ...
                >
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      ...
                      <small style={{ fontSize: 9, color: "#6f889c" }}>
                        {client?.service?.agent_name ? `${client.service.agent_name} · ` : ""}{stage}
                      </small>
                    </div>
                    <span
                      style={{
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    >
                      <div className="aiw-card-top">
                        <span className={`aiw-priority p-${item.priority.toLowerCase()}`}>
                          {PRIORITY_LABEL[item.priority] || item.priority}
                        </span>
                        <span className="aiw-source">
                          {item.source === "CENTRAL" ? "Central" : "ClickUp"}
                        </span>
                      </div>
                      <h3>{item.title}</h3>
                      <div className="aiw-card-foot">
                        <span className={isOverdue(item) ? "late" : ""}>
                          {item.due_at ? formatDate(item.due_at) : "Sem prazo"}
                        </span>
                        {item.events?.filter((event) => event.event_type === "COMMENT").length
                          ? <span>💬 {item.events.filter((event) => event.event_type === "COMMENT").length}</span>
                          : null}
                      </div>
                    </article>)}
                  </div>
    ...
    >
      <aside className="aiw-drawer">
        ...
        <div className="aiw-detail-grid">
          <label>
            ...
            >
              ...
              {selected.status === "COMPLETED" && <option value="COMPLETED">Concluída</option>}
            </select>
          </label>
          ...
          <label>
            ...
            <select
              ...
              disabled={busy === selected.id}
              onChange={(event) => updateItem(selected, { priority: event.target.value })}
            >
              {PRIORITIES.map((priority) => <option key={priority} value={priority}>
                {PRIORITY_LABEL[priority]}
              </option>)}
          ...
          <label>
            ...
            <input
              type="datetime-local"
              value={localInput(selected.due_at)}
              disabled={busy === selected.id}
              onChange={(event) => updateItem(selected, {
                due_at: event.target.value ? new Date(event.target.value).toISOString() : null,
              })}
            />
          </label>

          <div className="aiw-detail-source">
            <small>ORIGEM</small>
            <b>{selected.source === "CENTRAL" ? "Central de Trabalho" : "ClickUp → Central IA"}</b>
          </div>
        </div>

        <section className="aiw-detail-section">
          <h3>Contexto</h3>
          <p>{selected.description || "Sem contexto adicional registrado."}</p>
          {selected.clickup_url && <a href={selected.clickup_url} target="_blank" rel="noreferrer">
            Abrir origem no ClickUp ↗
          </a>}
        </section>

        {selected.status !== "COMPLETED" && <section className="aiw-detail-section">
          <h3>Concluir demanda</h3>
          <textarea
            value={completion}
            onChange={(event) => setCompletion(event.target.value)}
            placeholder="Registre o que foi feito / resultado da correção…"
          />
          <button
            className="aiw-success"
            disabled={busy === selected.id || !completion.trim()}
        ...
        {selected.resolution && <section className="aiw-detail-section aiw-resolution">
          <h3>Conclusão</h3>
        ...
        <section className="aiw-detail-section">
          ...
          <div className="aiw-comment-box">
            <textarea
              value={comment}
              onChange={(event) => setComment(event.target.value)}
              placeholder="Adicionar comentário, decisão, retorno do cliente…"
            />
            <button disabled={busy === selected.id || !comment.trim()} onClick={addComment}>
              Comentar
            </button>
          </div>
          ...
          <div className="aiw-events">
            {selected.events?.map((event) => <article key={event.id}>
              <div>
                <b>{event.actor_person || "Sistema"}</b>
              ...
              <p>
                {event.event_type === "COMMENT"
                  ...
                  : event.previous_status
                    ...
                    ? `${STATUS_LABEL[event.previous_status] || event.previous_status} → ${STATUS_LABEL[event.new_status] || event.new_status}${event.detail ? ` · ${event.detail}` : ""}`
                    : event.detail || event.event_type}
              </p>
            </article>)}
            {!selected.events?.length && <div className="aiw-no-events">
              Ainda sem comentários ou alterações registradas.
            </div>}
          </div>
        </section>
      </aside>
