module Api::V1::Accounts::Concerns::WhatsappHealthManagement
  ...
  included do
    ...
    before_action :check_admin_authorization?, only: [:register_webhook, :create_message_template]
    before_action :validate_whatsapp_cloud_channel, only: [:health, :register_webhook, :create_message_template]
  end
  ...
  def create_message_template
    result = Whatsapp::MessageTemplateCreationService.new(whatsapp_channel, message_template_params).perform
  ...
  def message_template_params
    params.require(:template).permit(:name, :language, :category, :parameter_format, :body)
  end
