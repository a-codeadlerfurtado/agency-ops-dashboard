...
module MetaTokenVerifyConcern
  ...
  def valid_meta_signature?
    ...
    return broker_signature_valid?(signature) if broker_signature_verification_configured?
  ...
  def broker_signature_verification_configured?
    ENV['META_BROKER_URL'].present? && ENV['META_BROKER_TOKEN'].present?
  end

  def broker_signature_valid?(signature)
    url = ENV.fetch('META_BROKER_URL').sub(%r{/exchange\z}, '/verify-signature')
    response = HTTParty.post(
      url,
      headers: { 'Authorization' => "Bearer #{ENV.fetch('META_BROKER_TOKEN')}", 'Content-Type' => 'application/json' },
      body: { signature: signature, bodyB64: Base64.strict_encode64(meta_request_body) }.to_json
    )
    response.success? && response.parsed_response.is_a?(Hash) && response.parsed_response['valid'] == true
  rescue StandardError => e
    Rails.logger.error "[WHATSAPP] Broker signature validation failed: #{e.message}"
    false
  end
