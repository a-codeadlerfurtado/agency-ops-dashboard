...
const FULL_GREETING_B64 = [GREETING_AUDIO_V5_00,GREETING_AUDIO_V5_01,GREETING_AUDIO_V5_02,GREETING_AUDIO_V5_03,GREETING_AUDIO_V5_04,GREETING_AUDIO_V5_05,GREETING_AUDIO_V5_06,GREETING_AUDIO_V5_07,GREETING_AUDIO_V5_08,GREETING_AUDIO_V5_09].join("");
const AUDIO_DATA_URL = `data:audio/ogg;base64,${FULL_GREETING_B64}`;

type Greeting = { show:boolean; date:string; is_monday:boolean; person:string; first_name:string };
...
function SpeakingOrb({ speaking, finished }: { speaking:boolean; finished:boolean }) {
  return <div className={`opsq3-orb ${speaking?"speaking":""} ${finished?"finished":""}`} aria-hidden="true">
    <i className="wave w1"/><i className="wave w2"/><i className="wave w3"/>
    <div className="core"><svg viewBox="0 0 100 100"><defs><radialGradient id="opsq3sphere" cx="38%" cy="34%" r="68%"><stop offset="0%" stopColor="#225d86"/><stop offset="52%" stopColor="#0a304a"/><stop offset="100%" stopColor="#020b12"/></radialGradient></defs><circle cx="50" cy="50" r="46" fill="url(#opsq3sphere)" stroke="#235b7d" strokeWidth="1"/><g fill="none" stroke="#7ec4e8" strokeWidth=".72" opacity=".72"><path d="M19 39 L31 30 L40 37 L52 23 L65 33 L78 29"/><path d="M15 54 L30 49 L40 58 L52 49 L64 55 L82 46"/><path d="M22 69 L34 61 L47 70 L58 60 L73 67"/><path d="M31 30 L30 49 L34 61 M40 37 L40 58 L47 70 M52 23 L52 49 L58 60 M65 33 L64 55 L73 67"/></g><g fill="#a9dcf5">{[[19,39],[31,30],[40,37],[52,23],[65,33],[78,29],[15,54],[30,49],[40,58],[52,49],[64,55],[82,46],[22,69],[34,61],[47,70],[58,60],[73,67]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r={i%4===0?1.35:.8}/>)}</g></svg></div>
...
export default function DailyGreetingV3() {
  ...
  const stopProgressTimer=()=>{ if(progressTimerRef.current!==null){window.clearInterval(progressTimerRef.current);progressTimerRef.current=null;} };

  const markStarted=(duration:number,currentTime:()=>number)=>{
    endingNaturallyRef.current=true;
    setError(null); setFinished(false); setProgress(0); setPlaying(true); setLoading(false);
    stopProgressTimer();
    progressTimerRef.current=window.setInterval(()=>setProgress(Math.min(1,currentTime()/Math.max(.1,duration||22.77))),100);
  };

  const markEnded=()=>{
    stopProgressTimer(); setPlaying(false);
    if(endingNaturallyRef.current){setProgress(1);setFinished(true);}
  };

  useEffect(()=>{
    const audio=new Audio(AUDIO_DATA_URL);
    audio.preload="auto"; audio.volume=1; audio.muted=true;
    htmlAudioRef.current=audio;

    const prime=()=>{
      if(primedRef.current) return;
      try{
        audio.muted=true; audio.volume=1; audio.currentTime=0;
        void audio.play().then(()=>{primedRef.current=true;}).catch(()=>undefined);
      }catch{}
    };
    ...
    return()=>{
      ...
      audio.pause(); audio.src=""; htmlAudioRef.current=null;
    };
  },[]);
  ...
  useEffect(()=>{
    ...
    async function claim(session:Session|null){
      ...
      try{
        const response=await authenticatedFetch(GREETING_API,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"CLAIM"}),cache:"no-store"});
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{
      if(!session){
        claimedUser.current=null; primedRef.current=false;
        const audio=htmlAudioRef.current; if(audio){audio.pause();audio.muted=true;try{audio.currentTime=0;}catch{}}
      }
      claim(session);
    });
  ...
  useEffect(()=>{
    ...
    if(!audio||!primedRef.current||audio.paused) return;
    try{
      audio.currentTime=0; audio.muted=false; audio.volume=1;
      audio.onended=markEnded;
      markStarted(Number.isFinite(audio.duration)?audio.duration:22.77,()=>audio.currentTime);
    }catch{}
  },[greeting?.date,greeting?.is_monday]);

  useEffect(()=>()=>{
    stopProgressTimer(); endingNaturallyRef.current=false;
    try{sourceRef.current?.stop();}catch{} sourceRef.current=null;
    const ctx=ctxRef.current;ctxRef.current=null;if(ctx&&ctx.state!=="closed") void ctx.close().catch(()=>undefined);
    const audio=htmlAudioRef.current;if(audio){audio.pause();audio.onended=null;}
  },[]);
  ...
  const startPlayback=async()=>{
    ...
    if(audio){
      try{
        endingNaturallyRef.current=false; audio.pause(); audio.muted=false; audio.volume=1; audio.currentTime=0;
        audio.onended=markEnded;
        await audio.play();
    ...
    try{
      const oldCtx=ctxRef.current;ctxRef.current=null;if(oldCtx&&oldCtx.state!=="closed") await oldCtx.close().catch(()=>undefined);
      const Ctor=getAudioContextCtor(); if(!Ctor) throw new Error("Saída de áudio não suportada neste navegador.");
      const ctx=new Ctor();ctxRef.current=ctx;await ctx.resume();
      if(ctx.state!=="running") throw new Error("O navegador bloqueou o áudio. Clique novamente em tocar.");
      const decoded=await ctx.decodeAudioData(decodeBundledAudio().slice(0));
      const source=ctx.createBufferSource();const gain=ctx.createGain();gain.gain.value=1;source.buffer=decoded;source.connect(gain);gain.connect(ctx.destination);sourceRef.current=source;
      source.onended=markEnded;source.start(0);
      const startedAt=ctx.currentTime;markStarted(decoded.duration,()=>Math.max(0,ctx.currentTime-startedAt));
    }catch(e){setLoading(false);setPlaying(false);setError(e instanceof Error?e.message:"Não foi possível iniciar a voz.");}
  };
