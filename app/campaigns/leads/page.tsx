...
export default function LeadConferencePage() {
  ...
  const rows = useMemo(() => {
    ...
    return (meta?.clients || []).map((client:Row) => {
      ...
      const rank:Record<string,number> = { NO_DISPATCH:0, ZERO:1, DIVERGENT:2, PARTIAL:3, NO_META:4, NO_ACTIVITY:5, OK:6 };
      return (rank[a.status] ?? 9)-(rank[b.status] ?? 9) || a.meta_leads-b.meta_leads || String(a.display_name).localeCompare(String(b.display_name),"pt-BR");
    });
  },[meta,dispatchClient,query,filter,lowThreshold]);
  ...
  return <main className="lc-shell"><style>{styles}</style>
    ...
    <div className="lc-period">{range.label} · {dateLabel(range.since)}{range.since!==range.until?` → ${dateLabel(range.until)}`:""}</div>
    ...
    <section className="lc-table">
      ...
      {rows.map((row:Row)=>{const open=expanded===String(row.client_id);const campaigns=campaignRows(row);const cpl=row.meta_leads>0?Number(row.spend||0)/row.meta_leads:null;return <article className={`lc-client ${open?"open":""}`} key={row.client_id}>
        <button className="lc-client-main" onClick={()=>setExpanded(open?null:String(row.client_id))}>
          <span className="lc-name"><b>{row.display_name}</b><small>{row.gt_owner||"Sem GT"} · {deliveryLabel[row.delivery_status]||row.delivery_status||"—"}</small></span>
