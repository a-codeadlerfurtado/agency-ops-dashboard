...
function signalGroups(detail: Row) {
  ...
  const defs = [
    ["Oportunidades", "✦", raw.opportunities || raw.oportunidades], ["Insights", "ϟ", raw.insights],
    ["Ideias", "◉", raw.ideas || raw.ideias], ["Objetivos", "◎", raw.objectives || raw.objetivos],
    ["Problemas atuais", "△", raw.problems || raw.problemas], ["Lições aprendidas", "✓", raw.lessons || raw.licoes],
  ];
...
export default function MeetingsBridge() {
  ...
  useEffect(() => {
    ...
    const ensureButton = () => {
      ...
      if (!button) { button = document.createElement("button"); button.type = "button"; button.dataset.meetingsNav = "true"; button.className = "meetings-nav-button"; button.title = "Reuniões"; button.setAttribute("aria-label", "Reuniões"); button.textContent = "Reuniões"; container.appendChild(button); }
  ...
  const detailActions = detail ? [...asItems(detail.commitments), ...asItems(detail.decisions).slice(0, 3)] : [];
  const detailSignals = detail ? signalGroups(detail) : [];
  const topics = detail ? asItems(detail?.metadata?.summary_topics || detail?.ai_signals?.summary_topics || detail.decisions).slice(0, 8) : [];
  const keywords = detail ? asItems(detail?.metadata?.keywords || detail?.ai_signals?.keywords).slice(0, 20) : [];

  if (typeof document === "undefined") return <style dangerouslySetInnerHTML={{ __html: STYLE }} />;
  return <><style dangerouslySetInnerHTML={{ __html: STYLE }} />{open && createPortal(<section className="meetings-app" aria-label="Reuniões"><div className="meetings-inner">
    {!detail && <>
      <div className="meetings-top"><h1>Feed de reuniões</h1><div className="meetings-actions"><button className="m-btn primary" onClick={createPairing} disabled={pairingLoading}>{pairingLoading ? "Gerando…" : "Conectar captura"}</button><button className="m-btn" onClick={() => load(true, activeQuery)} disabled={loading}>Atualizar</button><button className="m-btn icon" onClick={() => setOpen(false)}>×</button></div></div>
      {pairingCode && <div className="m-pair"><div><strong>{pairingCode}</strong><small>Digite este código em Leonardo Meetings. {pairingExpires ? `Expira ${new Intl.DateTimeFormat("pt-BR", { timeStyle: "short" }).format(new Date(pairingExpires))}.` : "Expira em 15 minutos."}</small></div><button className="m-btn" onClick={() => navigator.clipboard?.writeText(pairingCode)}>Copiar</button></div>}
    ...
    {(detail || detailLoading) && <>{detailLoading && !detail ? <div className="m-empty">Carregando reunião…</div> : detail && <>
      ...
      <div className="m-detailgrid"><div><section className="m-section"><h2 className="m-sectiontitle">☷ Ações ({detailActions.length})</h2>{detailActions.length ? detailActions.map((item,index) => <div className="m-action" key={index}><button className="m-follow">Acompanhar?</button><div><div className="m-actiontext">{item}</div>{asItems(detail.decisions)[index] && <div className="m-context">💡 {asItems(detail.decisions)[index]}</div>}</div></div>) : <div className="m-card m-empty" style={{padding:30}}>Nenhuma ação encontrada.</div>}<button className="m-btn" style={{marginTop:8}}>＋ Nova ação…</button></section>
      <section className="m-section"><h2 className="m-sectiontitle">☷ Resumo em tópicos</h2><div className="m-card">{topics.length ? topics.map((item,index) => <div className="m-topic" key={index}><strong>{item.split(/[.:–-]/)[0]}</strong><p>{item}</p></div>) : <div style={{color:"#94a3b8",fontSize:11}}>Os tópicos aparecerão depois do processamento.</div>}</div></section></div>
      <div><section className="m-section"><h2 className="m-sectiontitle">✧ Highlights</h2>{detailSignals.length ? detailSignals.map((group) => <div className="m-highlightgroup" key={String(group.name)}><h4>{String(group.icon)} {String(group.name)} ({group.items.length})</h4>{group.items.map((item,index) => <div className="m-highlight" key={index}><strong>{item.slice(0,90)}</strong><p>{item}</p></div>)}</div>) : <div className="m-card" style={{color:"#94a3b8",fontSize:11}}>Highlights ainda não processados.</div>}</section><section className="m-section"><h2 className="m-sectiontitle">⌘ Mapa mental</h2><div className="m-card m-mind"><div className="m-mindroot">Reunião</div><div className="m-mindnodes">{(topics.length ? topics : asItems(detail.decisions)).slice(0,5).map((item,index) => <div className="m-mindnode" key={index}>{item.slice(0,100)}</div>)}</div></div></section></div></div>
      <section className="m-section"><h2 className="m-sectiontitle">♩ Transcrição da reunião</h2><div className="m-accordion"><div className="m-accrow">Notas reescritas pela Leonardo <span style={{float:"right",color:"#94a3b8"}}>⌄</span></div><div className="m-accrow"><b>Transcrição completa</b><span style={{float:"right"}}>⌃</span></div><div className="m-transcriptbody"><div className="m-transcripttoolbar"><input className="m-transcriptsearch" value={transcriptQuery} onChange={(event) => setTranscriptQuery(event.target.value)} placeholder="Procurar em transcrição…" /><button className="m-btn" onClick={() => { const text = shownSegments.map((s) => `[${stamp(s.started_ms)}] ${s.speaker_name}: ${s.text}`).join("\n\n"); const blob = new Blob([text], {type:"text/plain;charset=utf-8"}); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `${cleanTitle(detail).replace(/[^a-z0-9_-]+/gi,"-")}.txt`; a.click(); URL.revokeObjectURL(a.href); }}>⇩ Exportar</button></div>{keywords.length > 0 && <div className="m-keywords"><b style={{color:"#f1f5f9",fontSize:13}}>⚿ Palavras chave</b><br />{keywords.join(", ")}</div>}<div className="m-transcriptscroll">{shownSegments.length ? shownSegments.map((segment,index) => <article className="m-transcriptblock" key={`${segment.message_id || index}-${segment.sequence_no || index}`}><header><span className="m-transcriptwho">{String(segment.speaker_name || "Participante")}</span><span className="m-transcripttime">{stamp(segment.started_ms)}</span></header><p className="m-transcripttext">{String(segment.text || "")}</p></article>) : <div className="m-empty">Transcrição estruturada indisponível.</div>}</div></div></div></section>
    </>}</>}
