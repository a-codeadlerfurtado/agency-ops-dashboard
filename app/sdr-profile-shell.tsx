...
function CallAudioPlayer({sessionId}:{sessionId:string}) {
  ...
  const loadAudio=async()=>{
    ...
    try{
      const response=await authenticatedFetch(API+"?session_id="+encodeURIComponent(sessionId)+"&audio=1",{cache:"no-store"});
      if(!response.ok){
        const body=await response.json().catch(()=>({}));
        throw new Error(body?.detail||body?.error||("Áudio "+response.status));
      }
      const blob=await response.blob();
      if(!blob.size)throw new Error("A gravação veio vazia.");
      setSrc(URL.createObjectURL(blob));
    }catch(error){
      setError(error instanceof Error?error.message:"Não foi possível carregar a gravação.");
    }finally{setLoading(false);}
  };
  return <div className="sdr-audio-player">
    {!src?<button type="button" onClick={loadAudio} disabled={loading}>{loading?"Carregando gravação...":"▶ Ouvir gravação"}</button>
      :<><audio controls autoPlay preload="metadata" src={src}/><a href={src} download={"relato-ligacao-"+sessionId+".mp3"}>Baixar gravação</a></>}
    {error&&<span className="sdr-audio-error">{error}</span>}
  </div>;
}
    ...
    {detail&&<div className="sdr-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setDetail(null);}}>
      <section className="sdr-modal" role="dialog" aria-modal="true" aria-label="Detalhes da ligação">
        ...
        <div className="sdr-detail-section"><h3>Gravação</h3>
          {String(detail.audio_status||"").toUpperCase()==="READY"
            ?<CallAudioPlayer sessionId={String(detail.session_id||detail.id)}/>
