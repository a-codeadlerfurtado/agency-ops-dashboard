...
export default function FinancePage() {
  ...
  const rows = useMemo(() => {
    ...
    return (payload.rows || []).filter((row) => {
      const matchesQuery = !needle || [row.display_name, row.gt_owner, row.cs_owner, row.canonical_source].join(" ").toLocaleLowerCase("pt-BR").includes(needle);
  ...
  return <main className="finance-shell">
    ...
    <section className="finance-hero"><div><span className="finance-eyebrow">DIREÇÃO · FINANCEIRO</span><h1>Valores comerciais da carteira</h1><p>Mensalidade, implementação, prazo e condição comercial. Adler e Leonardo podem corrigir os termos consolidados; evidências continuam separadas dos valores confirmados.</p></div><div className="finance-generated">{payload.generated_at ? `Atualizado ${formatDate(payload.generated_at)}` : "Aguardando carga"}</div></section>
    {error && <div className="finance-error">{error}</div>}
    <section className="finance-kpis"><article><small>MRR confirmado</small><strong>{formatMoney(summary.monthly_total_confirmed)}</strong><span>{formatNumber(summary.monthly_confirmed, 0)} de {formatNumber(current, 0)} clientes · {monthlyCoverage}% cobertura</span></article><article><small>Implementações confirmadas</small><strong>{formatMoney(summary.implementation_total_confirmed)}</strong><span>{formatNumber(summary.implementation_confirmed, 0)} clientes · {implementationCoverage}% cobertura</span></article><article><small>Prazos confirmados</small><strong>{formatNumber(summary.term_confirmed, 0)}</strong><span>de {formatNumber(current, 0)} clientes atuais</span></article><article><small>Em validação</small><strong>{formatNumber(Number(summary.monthly_candidates || 0) + Number(summary.implementation_candidates || 0), 0)}</strong><span>evidências provisórias</span></article></section>
    <section className="finance-panel"><div className="finance-panel-head"><div><h2>Carteira financeira</h2><p>{rows.length} clientes no filtro · clique em Editar para alterar os termos comerciais.</p></div><div className="finance-controls"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar cliente ou responsável" /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="CURRENT">Ativos + onboarding</option><option value="ACTIVE">Ativos</option><option value="ONBOARDING">Onboarding</option></select><select value={coverage} onChange={(event) => setCoverage(event.target.value)}><option value="ALL">Toda a carteira</option><option value="COMPLETE">Completo</option><option value="MISSING">Com informação faltante</option><option value="CANDIDATE">Com candidato</option></select></div></div>
      <div className="finance-table-wrap"><table><thead><tr><th>Cliente</th><th>Status</th><th>Mensalidade</th><th>Implementação</th><th>Pagamento</th><th>Prazo</th><th>Fontes</th><th>Atualização</th><th></th></tr></thead><tbody>{rows.map((row) => { const sources = sourceBadges(row); return <tr key={row.client_id}><td><b>{text(row.display_name)}</b><small>GT: {text(row.gt_owner || "não vinculado")}</small></td><td><span className={`status ${String(row.lifecycle).toLowerCase()}`}>{row.lifecycle === "ONBOARDING" ? "Onboarding" : "Ativo"}</span></td><td>{displayValue(row.monthly_value, row.monthly_evidence?.provisional)}</td><td>{displayValue(row.implementation_value, row.implementation_evidence?.provisional)}</td><td>{row.implementation_payment ? text(row.implementation_payment) : <span className="missing">Não confirmado</span>}</td><td>{row.term_months != null ? <strong>{formatNumber(row.term_months, 0)} meses</strong> : <span className="missing">Não confirmado</span>}</td><td><div className="source-list">{sources.length ? sources.map((source) => <span key={source}>{sourceLabel(source)}</span>) : <span className="missing">Sem evidência</span>}</div></td><td>{row.commercial_updated_at ? <><span>{formatDate(row.commercial_updated_at)}</span><small>{text(row.commercial_updated_by || row.canonical_source || "evidências vinculadas")}</small></> : <span className="missing">Sem consolidação</span>}</td><td><button className="finance-refresh" onClick={() => openEditor(row)}>Editar</button></td></tr>; })}{!rows.length && <tr><td colSpan={9} className="finance-empty">Nenhum cliente nesse filtro.</td></tr>}</tbody></table></div>
    </section>
