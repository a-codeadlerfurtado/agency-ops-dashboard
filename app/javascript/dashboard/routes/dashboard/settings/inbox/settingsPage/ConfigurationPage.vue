...
export default {
  ...
  methods: {
    ...
    async createMessageTemplate() {
      ...
      try {
        await InboxesAPI.createMessageTemplate(this.inbox.id, {
          name: this.templateName.trim(),
          language: this.templateLanguage.trim(),
...
<template>
  ...
  <div v-else-if="isAWhatsAppChannel && !isATwilioChannel">
    <div v-if="inbox.provider_config">
      ...
      />
      <SettingsFieldSection
        v-if="inbox.provider === 'whatsapp_cloud'"
        label="Criar template na Meta"
        help-text="Crie um template de mensagem e envie para aprovação sem sair da ImoBia."
      >
        <div class="flex flex-col gap-3 w-full">
          <woot-input v-model="templateName" label="Nome" placeholder="ex: retorno_lead" />
          <div class="grid grid-cols-2 gap-3">
            <woot-input v-model="templateLanguage" label="Idioma" placeholder="pt_BR" />
            <select v-model="templateCategory" class="h-10 rounded-lg border border-n-weak bg-n-solid-1 px-3 text-sm text-n-slate-12">
              <option value="UTILITY">Utilidade</option>
              <option value="MARKETING">Marketing</option>
              <option value="AUTHENTICATION">Autenticação</option>
            </select>
          </div>
          <TextArea v-model="templateBody" placeholder="Texto do template" auto-height resize />
          <div class="flex justify-end">
            <NextButton
              :is-loading="isCreatingTemplate"
