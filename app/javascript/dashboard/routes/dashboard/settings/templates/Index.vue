...
const createInboxId = ref('');
const createName = ref('');
const createLanguage = ref('pt_BR');
const createCategory = ref('UTILITY');
...
const cloudWhatsAppInboxes = computed(() =>
  whatsappInboxes.value.filter(
    inbox => inbox.channel_type === INBOX_TYPES.WHATSAPP && inbox.provider === 'whatsapp_cloud'
  )
);
...
const openCreateTemplate = () => {
  if (!createInboxId.value && cloudWhatsAppInboxes.value.length) {
    createInboxId.value = String(cloudWhatsAppInboxes.value[0].id);
  }
  showCreate.value = true;
};

const createTemplate = async () => {
  if (!createInboxId.value || !createName.value.trim() || !createBody.value.trim()) return;
  isCreating.value = true;
  try {
    const inboxId = Number(createInboxId.value);
    await InboxesAPI.createMessageTemplate(inboxId, {
      name: createName.value.trim(),
      language: createLanguage.value.trim() || 'pt_BR',
    ...
    useAlert('Template enviado para aprovação da Meta.');
    await fetchTemplates();
  } catch (error) {
    useAlert(error.response?.data?.error || 'Não foi possível criar o template.');
  } finally {
...
<template>
  <SettingsLayout
    ...
    :no-records-found="!templates.length && !showCreate"
  ...
  >
      ...
      >
        ...
        <template #actions>
          <Button
            ...
            color="blue"
            size="sm"
            :disabled="!cloudWhatsAppInboxes.length"
    ...
    <template #body>
      <div v-if="showCreate" class="m-4 rounded-xl border border-n-weak bg-n-solid-1 p-5">
        <div class="mb-4 flex items-center justify-between">
          <div>
            <h3 class="text-base font-semibold text-n-slate-12">Novo template WhatsApp</h3>
            <p class="text-sm text-n-slate-10">A Meta revisa o template antes de liberar o envio.</p>
          </div>
          <Button label="Fechar" color="slate" size="sm" @click="showCreate = false" />
        </div>
        <div class="grid gap-3 md:grid-cols-2">
          <label class="text-sm text-n-slate-11">Inbox
            <select v-model="createInboxId" class="mt-1 h-10 w-full rounded-lg border border-n-weak bg-n-solid-1 px-3 text-n-slate-12">
              <option v-for="inbox in cloudWhatsAppInboxes" :key="inbox.id" :value="String(inbox.id)">{{ inbox.name }}</option>
            </select>
          </label>
          <label class="text-sm text-n-slate-11">Nome
            <input v-model="createName" class="mt-1 h-10 w-full rounded-lg border border-n-weak bg-n-solid-1 px-3 text-n-slate-12" placeholder="ex: retorno_lead" />
          </label>
          <label class="text-sm text-n-slate-11">Idioma
            <input v-model="createLanguage" class="mt-1 h-10 w-full rounded-lg border border-n-weak bg-n-solid-1 px-3 text-n-slate-12" placeholder="pt_BR" />
          </label>
          <label class="text-sm text-n-slate-11">Categoria
            <select v-model="createCategory" class="mt-1 h-10 w-full rounded-lg border border-n-weak bg-n-solid-1 px-3 text-n-slate-12">
        ...
        <label class="mt-3 block text-sm text-n-slate-11">Texto
          <textarea v-model="createBody" rows="5" class="mt-1 w-full rounded-lg border border-n-weak bg-n-solid-1 p-3 text-n-slate-12" placeholder="Digite o conteúdo do template"></textarea>
        </label>
        <div class="mt-4 flex justify-end"><Button label="Enviar para aprovação" color="blue" :is-loading="isCreating" :disabled="isCreating || !createInboxId || !createName.trim() || !createBody.trim()" @click="createTemplate" /></div>
      </div>

      <div
        v-if="!filteredTemplates.length && !showCreate"
