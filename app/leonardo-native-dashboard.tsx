  ...
  return <div className="leo-native-stack">
    ...
    <section className="leo-native-grid2">
      <article className="card section">
        <div className="panel-heading"><div><span className="eyebrow">Pipeline</span><h3>Etapas do funil</h3></div></div>
        {stages.length ? stages.map((row) => <div className="leo-native-row" key={row.stage}><span><b>{stageLabel[row.stage] || text(row.stage)}</b><small>{number(row.count)} oportunidades</small></span><strong>{money(row.weighted_value)}</strong></div>) : <div className="empty">Sem oportunidades no funil.</div>}
      </article>
      <article className="card section">
        <div className="panel-heading"><div><span className="eyebrow">Time comercial</span><h3>Resultado por responsável</h3></div></div>
        {performance.length ? performance.map((row) => <div className="leo-native-row" key={row.owner_id || row.owner_name}><span><b>{text(row.owner_name)}</b><small>{number(row.open_leads)} abertos · {number(row.proposals)} propostas · {number(row.negotiations)} negociações</small></span><span className="leo-native-right"><strong>{money(row.weighted_value)}</strong><small>{number(row.won_30d)} fechados em 30d</small></span></div>) : <div className="empty">Sem responsáveis comerciais encontrados.</div>}
      </article>
    </section>
  </div>;
}
...
export default function LeonardoNativeDashboard({ session }: { session: Session }) {
  ...
  useEffect(() => {
    ...
    try { window.localStorage.setItem("ops-theme", theme); } catch { /* ignore */ }
  }, [theme]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Falha ao carregar dados comerciais.");
    } finally {
      setLoading(false);
    }
  ...
  const saveProfile = async (event: React.FormEvent) => {
    ...
    try {
      const { error: updateError } = await supabase.auth.updateUser({ data: { name: next, full_name: next } });
  ...
  const openItem = (item: NavItem) => {
    ...
    if (item.tab) { setTab(item.tab); setProfileOpen(false); setNotificationsOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); }
  };
  ...
  return <>
    <main className={`shell ${sidebarOpen ? "sidebar-open" : "sidebar-collapsed"}`}>
      ...
      <div className="source-banner"><span>Visão executiva comercial.</span> Comercial, financeiro e onboarding permanecem acessíveis; a operação geral continua fora deste perfil.</div>
      ...
      <aside className={`side-nav${sidebarOpen ? " open" : ""}`} aria-label="Visões do dashboard">
        ...
        <div className="side-nav-items">{NAV.map((item) => <button key={item.key} className={item.tab === tab && !item.href ? "active" : ""} onClick={() => openItem(item)} title={item.label}>{item.label}</button>)}</div>
      </aside>

      {error && <div className="error-box">{error}</div>}
      {loading && !Object.keys(data).length && tab !== "creative" ? <div className="auth-loading"><span className="dot loading"/> Carregando Central Comercial…</div> : <CommercialContent tab={tab} data={data} token={session.access_token} />}
    </main>
    ...
    <style>{`
      .leo-native-stack{display:grid;gap:14px}.leo-native-kpis{grid-template-columns:repeat(6,minmax(130px,1fr))}.leo-native-direction-kpis{grid-template-columns:repeat(4,minmax(150px,1fr))}
      .leo-native-sources{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-bottom:0}.leo-native-source>div{display:flex;align-items:center;gap:8px}.leo-native-source strong,.leo-native-source small{display:block;margin-top:7px}.leo-native-source small{color:var(--muted)}
      ...
      @media(max-width:1100px){.leo-native-kpis{grid-template-columns:repeat(3,minmax(130px,1fr))}.leo-native-sources,.leo-native-grid2{grid-template-columns:1fr}.live>span[role=status]{display:none}}
      @media(max-width:720px){.leo-native-kpis,.leo-native-direction-kpis{grid-template-columns:repeat(2,minmax(120px,1fr))}.command-trigger{display:none}.leo-native-command-grid{grid-template-columns:1fr}}
    `}</style>
  </>;
}
