...
export default function JarvisVoice({ allowed }: { allowed?: boolean } = {}) {
  ...
  const [cedarVerificado, setCedarVerificado] = useState(false);
  ...
  const falarResposta = useCallback(async (texto: string) => {
    ...
    try {
      ...
      if (engine !== "openai" || voice !== "cedar" || !["cedar-only-v3", "cedar-only-v4"].includes(ttsBuild)) {
        throw new Error(`voz_nao_cedar:${engine || "sem-engine"}:${voice || "sem-voice"}:${ttsBuild || "sem-build"}`);
      }
      ...
      audio.onerror = () => {
        encerrar();
        if (geracaoAudioRef.current === geracao) setErro("Cedar indisponível (falha de reprodução)");
      };
      await audio.play();
    } catch (erroTts) {
      if (geracaoAudioRef.current !== geracao) return;
      ...
      setErro(`Cedar indisponível (${motivo})`);
    }
  }, [calar]);
  ...
  const ouvir = useCallback(() => {
    ...
    // Desbloqueia o AudioContext dentro do gesto do usuario. Assim, quando o
    // primeiro chunk PCM Cedar chegar, ele pode tocar sem esperar nova interacao.
    try {
      const AudioCtx = window.AudioContext || (window as any).webkitAudioContext;
    ...
    // Esconde o custo de identidade/carteira/catalogo dentro do tempo em que a pessoa esta falando.
    const tokenWarm = sessaoRef.current?.access_token;
    if (tokenWarm) {
      void fetch("/api/jarvis/warm-lite", {
        method: "POST", headers: { authorization: `Bearer ${tokenWarm}` },
      }).catch(() => null);
    }
  ...
  return (
    <>
      ...
      <div style={cabecalho}>
        ...
        <span style={{ marginLeft: 8, fontSize: 10, fontWeight: 700, opacity: cedarVerificado ? 1 : 0.7, color: cedarVerificado ? "var(--green)" : "var(--muted)" }}>
          {cedarVerificado ? "Cedar ✓" : "Cedar only"}
        </span>
