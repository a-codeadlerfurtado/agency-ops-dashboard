...
const MODE:Record<string,string>={PLANTAO:"Por plantão",GRUPO_MANUAL:"Portal + WhatsApp",CRM_INTEGRADO:"CRM integrado",HIBRIDO:"CRM + portal"};
...
export function CommercialFollowupCenter({ token }: { token:string }) {
  ...
  return <section className="cfc-shell">
    ...
    {!loading&&selected==="view-imoveis"&&<section className="cfc-client-wrap"><ClientModeStrip client={selectedClient||viewClient}/><ViewOncallCenter token={token}/></section>}
...
function ClientModeStrip({client}:{client?:Row}){
  if(!client)return null;
  return <div className="cfc-mode-strip">
    <div><span>Modo</span><b>{MODE[client.collection_mode]||client.collection_mode}</b></div>
    <div><span>Automação</span><b className={client.automation_enabled?"good":"muted"}>{client.automation_enabled?"Ativa":"Desativada"}</b></div>
    {client.group_chat_name&&<div><span>Grupo comercial</span><b>{client.group_chat_name}</b></div>}
    <div><span>Origem principal</span><b>{client.collection_mode==="PLANTAO"?"Colibra + WhatsApp":client.collection_mode==="CRM_INTEGRADO"?"CRM/API":"Portal + WhatsApp"}</b></div>
  </div>;
}
...
function ResponseCard({entry,client}:{entry:Row;client:Row}){
  ...
  return <article className="cfc-response"><header><div><b>{b.display_name||"Corretor"}</b><span>Respondido {when(entry.responded_at||entry.first_response_at)}</span></div>{anomaliesList.length>0&&<i>⚠ revisar</i>}</header>
    <div className="cfc-metric-grid">{keys.map(k=>{const p=entry.provenance?.[k]; const v=entry.metrics?.[k]; return <div key={k}><span>{METRICS[k]||k}</span><b>{v===undefined||v===null?"—":v}</b>{p&&<small>{p.source==="DECLARADO_WHATSAPP"?"WhatsApp":p.source==="AJUSTE_MANUAL"?"Ajuste manual":p.source}</small>}</div>})}</div>
...
function ConfigPanel({client,data,role,call,busy}:{client:Row;data:Payload;role:string;call:(b:Row)=>Promise<any>;busy:string}){
  ...
  return <section className="cfc-card cfc-config"><header><div><span>CONFIGURAÇÃO</span><h3>Regra deste cliente</h3></div></header>
    <div className="cfc-config-grid">
      <label>Modo de coleta<select value={form.collection_mode||"GRUPO_MANUAL"} onChange={e=>setForm((f:Row)=>({...f,collection_mode:e.target.value}))}><option value="GRUPO_MANUAL">Portal + apoio no WhatsApp</option><option value="CRM_INTEGRADO">CRM integrado</option><option value="HIBRIDO">CRM + portal</option></select></label>
