...
Deno.serve(async (req: Request) => {
  ...
  if (view !== "home") return reply({ error: "forbidden", scope: "COMMERCIAL_MANAGER_NO_GENERAL_OPS" }, 403);

  return reply({
    kpis: {}, clients: [], alerts: [], commitments: [], conversations: [], media: {},
    ...
    campaigns: [], notifications: [], preclients: [], won_events: [], audit_runs: [], audit_issues: [], integration_health: [], team: [], unassigned_clients: [], portfolio: null, wallets: [], adjustments: [], stage_labels: {}, access_requests_pending: [],
    preferences: {
      name: preference?.name || "Leonardo Augusto", role: "Direção Comercial", theme: preference?.theme || "dark",
      sounds_enabled: preference?.sounds_enabled, win_sound_enabled: preference?.win_sound_enabled,
      win_celebration_enabled: preference?.win_celebration_enabled, notifications_enabled: preference?.notifications_enabled,
      animations_enabled: preference?.animations_enabled, interface_density: preference?.interface_density,
    },
    profile: {
      person: "Leonardo Augusto",
      role: "COMMERCIAL",
      display_role: "Direção Comercial",
      access_level: "COMMERCIAL_MANAGER",
      ...
      can_use_assigned_work: true,
      can_use_assigned_creative: true,
      can_use_diary: true,
      can_use_tasklog: true,
      is_executive: true,
      locked: false,
      account_approved: true,
      views: ["overview", "clients", "campaigns", "commercial_direction", "finance", "onboarding", "creative", "work", "diary"],
      views_stale: false,
      carteira: null,
