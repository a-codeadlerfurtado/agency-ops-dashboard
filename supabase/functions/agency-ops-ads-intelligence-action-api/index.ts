...
async function metaJson(url: string, init?: RequestInit) {
  ...
  if (!response.ok || body?.error) { const error = new Error(body?.error?.message || `Meta HTTP ${response.status}`); (error as any).metaCode = body?.error?.code; throw error; }
  return body;
}
...
async function objectState(token: string, objectType: string, id: string) {
  const candidates = objectType === "ADSET" ? [
    "id,name,status,effective_status,account_id,campaign_id,daily_budget,lifetime_budget,optimization_goal,bid_strategy",
    "id,name,status,effective_status,account_id,campaign_id,daily_budget,lifetime_budget,optimization_goal",
    "id,name,status,effective_status,account_id,campaign_id,daily_budget,lifetime_budget",
    "id,name,status,account_id,campaign_id",
  ] : [
    "id,name,status,effective_status,account_id,campaign_id,adset_id",
    "id,name,status,account_id,campaign_id,adset_id",
  ];
  let last: unknown = null;
  for (const fields of candidates) {
    try {
      return await metaJson(`https://graph.facebook.com/${VERSION}/${encodeURIComponent(id)}?fields=${encodeURIComponent(fields)}&access_token=${encodeURIComponent(token)}`);
    } catch (error) { last = error; }
  }
  throw last || new Error("Meta object read failed");
}
async function postObject(token: string, id: string, values: Record<string, string>) {
...
async function liveMtd(token: string, accounts: string[], since: string, until: string) {
  ...
  for (const account of accounts) {
    ...
    } catch { /* guard usa somente leituras confirmadas */ }
  }
...
Deno.serve(async (req: Request) => {
  ...
  if (!["SET_STATUS", "SET_DAILY_BUDGET"].includes(action)) return json({ error: "invalid_action" }, 400);
  if (action === "SET_DAILY_BUDGET" && objectType !== "ADSET") return json({ error: "budget_only_for_adset" }, 400);
  ...
  let before: Row;
  try { before = await objectState(token, objectType, objectId); }
  catch (error) { return json({ error: "meta_read_failed", detail: clean((error as Error)?.message || error) }, 502); }
  const accountId = cleanAccount(before.account_id);
  if (!accountIds.includes(accountId)) return json({ error: "object_not_owned_by_client" }, 403);
  ...
  let desiredStatus = "", desiredBudgetCents: number | null = null, budgetGuard: Row | null = null;
  if (action === "SET_STATUS") {
    desiredStatus = clean(body.status).toUpperCase();
    if (!["ACTIVE", "PAUSED"].includes(desiredStatus)) return json({ error: "invalid_status" }, 400);
    const oldStatus = clean(before.status).toUpperCase();
    if (!["ACTIVE", "PAUSED"].includes(oldStatus)) return json({ error: "object_state_not_toggleable", status: oldStatus }, 409);
  } else {
    const requested = finite(body.daily_budget);
    if (requested === null || requested < 1 || requested > 1000000) return json({ error: "invalid_daily_budget" }, 400);
    ...
    desiredBudgetCents = Math.round(requested * 100);
    const oldCents = finite(before.daily_budget) || 0;
    const deltaDaily = Math.max(0, (desiredBudgetCents - oldCents) / 100);
    if (deltaDaily > 0) {
      const monthly = finite(term?.minimum_ad_budget);
      const today = localDate(), since = `${today.slice(0, 8)}01`;
      const mtd = await liveMtd(token, accountIds, since, today);
      const day = Number(today.slice(8, 10)), daysInMonth = new Date(Number(today.slice(0, 4)), Number(today.slice(5, 7)), 0).getDate();
      const remainingDays = Math.max(1, daysInMonth - day);
      const remaining = monthly !== null && mtd !== null ? Math.max(0, monthly - mtd) : null;
      const currentPace = mtd !== null ? mtd / Math.max(1, day) : null;
      const idealRemaining = remaining !== null ? remaining / remainingDays : null;
      const extraRoom = idealRemaining !== null && currentPace !== null ? Math.max(0, idealRemaining - currentPace) : null;
      budgetGuard = { monthly_budget: monthly, mtd_spend: mtd, remaining_budget: remaining, remaining_days: remainingDays, current_daily_pace: currentPace, ideal_daily_remaining: idealRemaining, requested_daily_increase: deltaDaily, daily_increase_room: extraRoom };
      const guardFailed = monthly === null || mtd === null || remaining === null || remaining <= 0 || extraRoom === null || deltaDaily > Math.max(1, extraRoom * 1.15);
  ...
  const actionType = action === "SET_STATUS" ? `META_${objectType}_STATUS` : "META_ADSET_DAILY_BUDGET";
  const beforeState = [{ ...before }];
  const previewHash = await sha256(JSON.stringify({ actor: user.id, client: client.id, objectType, objectId, action, desiredStatus, desiredBudgetCents, before: beforeState }));
  ...
  const { data: audit, error: auditError } = await ops.from("safe_action_requests").insert({
    ...
    action_type: actionType, target_ids: [objectId], preview_state: beforeState, preview_hash: previewHash,
    before_state: beforeState, expires_at: new Date(Date.now() + 10 * 60_000).toISOString(), status: "EXECUTING", confirmed_at: now,
    request_metadata: { source: "ADS_INTELLIGENCE", object_type: objectType, object_name: before.name || null, budget_guard: budgetGuard },
  }).select("id").single();
  if (auditError || !audit?.id) return json({ error: "audit_store_failed" }, 500);

  try {
    if (action === "SET_STATUS") await postObject(token, objectId, { status: desiredStatus });
    else await postObject(token, objectId, { daily_budget: String(desiredBudgetCents) });
  } catch (error) {
  ...
  let after: Row;
  try { after = await objectState(token, objectType, objectId); }
  catch (error) {
  ...
  const verified = action === "SET_STATUS" ? clean(after.status).toUpperCase() === desiredStatus : finite(after.daily_budget) === desiredBudgetCents;
  if (!verified) {
    let rollbackOk = false;
    try {
      if (action === "SET_STATUS") await postObject(token, objectId, { status: clean(before.status).toUpperCase() });
      else await postObject(token, objectId, { daily_budget: String(Math.round(finite(before.daily_budget) || 0)) });
      const rollback = await objectState(token, objectType, objectId);
      rollbackOk = action === "SET_STATUS" ? clean(rollback.status).toUpperCase() === clean(before.status).toUpperCase() : finite(rollback.daily_budget) === finite(before.daily_budget);
    } catch { rollbackOk = false; }
    await ops.from("safe_action_requests").update({ status: rollbackOk ? "ROLLED_BACK" : "FAILED", executed_at: new Date().toISOString(), after_state: [after], verification: { verified: false, rollback_attempted: true, rollback_ok: rollbackOk }, error: "Meta returned a state different from the requested state." }).eq("id", audit.id);
    return json({ error: "verification_failed", rollback_ok: rollbackOk, audit_id: audit.id }, 502);
  }
  await ops.from("safe_action_requests").update({ status: "VERIFIED", executed_at: new Date().toISOString(), after_state: [after], verification: { verified: true, verified_at: new Date().toISOString() }, error: null }).eq("id", audit.id);
  return json({ ok: true, status: "VERIFIED", object_type: objectType, object_id: objectId, before, after, budget_guard: budgetGuard, audit_id: audit.id });
});
