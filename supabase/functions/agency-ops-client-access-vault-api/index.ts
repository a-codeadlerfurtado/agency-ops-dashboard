...
const VAULT_KEY_B64 = Deno.env.get("CLIENT_VAULT_KEY_B64") || "";
...
function cors(req: Request) {
  ...
  const allowedOrigin = !origin || ALLOWED_ORIGINS.has(origin) ? (origin || PROD_ORIGIN) : "null";
  return {
...
function ownerMatches(owner: unknown, person: string) {
  ...
  const personText = normalize(person);
  if (!ownerText || !personText) return false;
  if (ownerText === personText) return true;
  const parts = ownerText.split(/\s*(?:,|;|\||\/|&|\be\b)\s*/g).filter(Boolean);
  if (parts.includes(personText)) return true;
  return ownerText.includes(personText);
}
...
function encryptionKey() {
  if (!keyPromise) {
    keyPromise = (async () => {
      if (!VAULT_KEY_B64) throw new Error("vault_key_missing");
      const raw = b64ToBytes(VAULT_KEY_B64);
...
async function encrypt(value: string | null) {
  if (value === null || value === "") return { ciphertext: null, iv: null };
...
async function decrypt(ciphertext: unknown, iv: unknown) {
  ...
  const plaintext = await crypto.subtle.decrypt(
    { name: "AES-GCM", iv: b64ToBytes(String(iv)) },
    key,
    b64ToBytes(String(ciphertext)),
  );
...
async function identity(admin: ReturnType<typeof makeAdmin>, req: Request): Promise<Identity | null> {
...
async function resolveClient(admin: ReturnType<typeof makeAdmin>, body: Row): Promise<Client | null | "AMBIGUOUS"> {
  const id = cleanText(body.client_id, 80);
  if (id) {
  ...
  const name = cleanText(body.client_name, 240);
  if (!name) return null;
  const { data } = await admin
    .schema("agency_ops")
    .from("dashboard_client_overview")
...
function canManage(actor: Identity) {
  return actor.role === "MGMT";
}

async function audit(admin: ReturnType<typeof makeAdmin>, actor: Identity, client: Client, action: string, itemId?: string | null, detail: Row = {}) {
  const safeDetail: Row = {};
  for (const [key, value] of Object.entries(detail)) {
    if (["password", "login", "notes", "ciphertext", "iv", "dashboard_password"].includes(key)) continue;
    safeDetail[key] = typeof value === "string" ? value.slice(0, 500) : value;
  }
  await admin.schema("agency_ops").from("client_access_vault_audit").insert({
    client_id: client.client_id,
    vault_item_id: itemId || null,
...
async function isReauthLocked(admin: ReturnType<typeof makeAdmin>, actor: Identity) {
  ...
  return Number(count || 0) >= REAUTH_FAILURE_LIMIT;
}

async function reauthenticate(admin: ReturnType<typeof makeAdmin>, actor: Identity, client: Client, password: unknown) {
  if (await isReauthLocked(admin, actor)) {
  ...
  const { data, error } = await verifier.auth.signInWithPassword({ email: actor.email, password: supplied });
  if (error || !data.user || data.user.id !== actor.userId) {
...
Deno.serve(async (req) => {
  ...
  const actor = await identity(admin, req);
  ...
  const client = await resolveClient(admin, body);
  ...
  if (action === "LIST") {
    ...
    return json(req, {
      ...
      permissions: { role: actor.role, can_manage: canManage(actor), can_reveal: true },
      items: items || [],
      audit: auditRows,
    });
  }
  ...
  if (action === "REVEAL") {
    ...
    const { data: item, error } = await admin
      .schema("agency_ops")
      .from("client_access_vault")
      .select("id,client_id,category,system_name,login_url,login_ciphertext,login_iv,password_ciphertext,password_iv,notes_ciphertext,notes_iv,updated_at")
      .eq("id", itemId)
      .eq("client_id", client.client_id)
      .maybeSingle();
    if (error || !item) return json(req, { ok: false, error: "credential_not_found" }, 404);
    try {
      const [login, password, notes] = await Promise.all([
        decrypt(item.login_ciphertext, item.login_iv),
        decrypt(item.password_ciphertext, item.password_iv),
        decrypt(item.notes_ciphertext, item.notes_iv),
      ]);
      ...
      return json(req, { ok: false, error: "credential_decryption_failed" }, 500);
    }
  }

  if (action === "COPY") {
    if (!itemId) return json(req, { ok: false, error: "item_id_required" }, 400);
    const field = cleanText(body.field, 20).toUpperCase();
    if (!['LOGIN', 'PASSWORD'].includes(field)) return json(req, { ok: false, error: "invalid_copy_field" }, 400);
  ...
  if (action === "CREATE") {
    const reauth = await reauthenticate(admin, actor, client, body.dashboard_password);
    if (!reauth.ok) return json(req, { ok: false, error: reauth.error }, reauth.status);
    const category = cleanText(body.category, 40).toUpperCase() || "CRM";
    const systemName = cleanText(body.system_name, 120);
    const loginUrl = safeUrl(body.login_url);
    const loginValue = cleanText(body.login, 320);
    const passwordValue = String(body.password || "").slice(0, 1000);
    const notesValue = cleanText(body.notes, 2000);
    ...
    try {
      const [loginEnc, passwordEnc, notesEnc] = await Promise.all([
        encrypt(loginValue || null),
        encrypt(passwordValue),
        encrypt(notesValue || null),
      ]);
      const { data: created, error } = await admin
        ...
        .insert({
          ...
          login_url: loginUrl,
          login_ciphertext: loginEnc.ciphertext,
          login_iv: loginEnc.iv,
          password_ciphertext: passwordEnc.ciphertext,
          password_iv: passwordEnc.iv,
          notes_ciphertext: notesEnc.ciphertext,
          notes_iv: notesEnc.iv,
          encryption_version: 1,
          created_by_user_id: actor.userId,
          created_by_person: actor.person,
          updated_by_user_id: actor.userId,
          updated_by_person: actor.person,
        })
        .select("id,category,system_name,login_url,created_at,updated_at")
        .single();
  ...
  if (action === "UPDATE") {
    ...
    const { data: current } = await admin
      .schema("agency_ops")
      .from("client_access_vault")
      .select("id,client_id,category,system_name,login_url")
      .eq("id", itemId)
      .eq("client_id", client.client_id)
      .maybeSingle();
    ...
    if (Object.prototype.hasOwnProperty.call(body, "system_name")) {
      const name = cleanText(body.system_name, 120);
      if (!name) return json(req, { ok: false, error: "system_name_required" }, 400);
      patch.system_name = name;
    }
    if (Object.prototype.hasOwnProperty.call(body, "login_url")) {
      const url = safeUrl(body.login_url);
      if (body.login_url && !url) return json(req, { ok: false, error: "invalid_login_url" }, 400);
      patch.login_url = url;
    }
    try {
      if (body.clear_login === true) {
        patch.login_ciphertext = null;
        patch.login_iv = null;
      } else if (cleanText(body.login, 320)) {
        const value = await encrypt(cleanText(body.login, 320));
        patch.login_ciphertext = value.ciphertext;
        patch.login_iv = value.iv;
      }
      if (String(body.password || "")) {
        const value = await encrypt(String(body.password).slice(0, 1000));
        patch.password_ciphertext = value.ciphertext;
        patch.password_iv = value.iv;
      }
      if (body.clear_notes === true) {
        patch.notes_ciphertext = null;
        patch.notes_iv = null;
      } else if (cleanText(body.notes, 2000)) {
        const value = await encrypt(cleanText(body.notes, 2000));
        patch.notes_ciphertext = value.ciphertext;
        patch.notes_iv = value.iv;
      }
    } catch (error) {
      const code = error instanceof Error ? error.message : "encryption_failed";
      return json(req, { ok: false, error: code.startsWith("vault_key_") ? code : "credential_encryption_failed" }, 503);
    ...
    await audit(admin, actor, client, "UPDATE", itemId, {
      system_name: updated.system_name,
      category: updated.category,
      login_changed: Boolean(cleanText(body.login, 320) || body.clear_login === true),
      password_changed: Boolean(String(body.password || "")),
      notes_changed: Boolean(cleanText(body.notes, 2000) || body.clear_notes === true),
    });
  ...
  if (action === "DELETE") {
    ...
    const { data: current } = await admin
      .schema("agency_ops")
      .from("client_access_vault")
      .select("id,system_name,category")
      .eq("id", itemId)
      .eq("client_id", client.client_id)
      .maybeSingle();
    if (!current) return json(req, { ok: false, error: "credential_not_found" }, 404);
