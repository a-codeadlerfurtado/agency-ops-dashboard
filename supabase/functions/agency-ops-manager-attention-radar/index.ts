...
const SEVERE_RE = /(nao entra|nao chega|nao vai|sem lead|sem leads|erro|problema|urgente|complicado|parado|nao funciona|nao esta funcionando|nada)/;
const PROGRESS_RE = /(vou |vamos |iremos |irei |podemos enviar|assim que|solicitei|repass|vou pedir|irei solicitar|estamos resolvendo|faremos|ajustaremos|vamos ajustar|vamos subir|irei trazer|vamos trazer|vou enviar|iremos enviar|vou analisar|vou verificar|iremos fazer|esta resolvendo|estamos verificando|irei averiguar)/;
const CLOSURE_RE = /(feito|feita|corrig|ajustad|alterad|subid|publicad|ativad|conectad|enviad|finalizad|resolvid|realizad|cobrad|ja esta|ja foi|campanha .* no ar|numero conectado|segue o relatorio|segue a previa|segue os criativos|foi subido|foi alterado|foi corrigido|esta no ar|estao no ar)/;
const DEPENDENCY_RE = /(preencher|nos enviar|me enviar|pode enviar|precisamos de|aguardando|assim que receber|quando enviar|falta .* cliente|depende .* cliente|retorno de voces|pagamento|abastecimento|aprovar|aprovacao|poderia checar|poderia confirmar|consegue checar|consegue confirmar|confirma para mim|checar para mim)/;
...
function isActionableClient(m: any) {
  const raw = String(m?.body || "").trim();
  const q = norm(raw);
  if (q.length < 10) return false;
  if (/^precisando$/.test(q)) return false;
  if (/trocar ideias|trocando ideias/.test(q)) return false;
  if (SEVERE_RE.test(q)) return true;
  ...
  if (/o que e preciso|de uma olhada/.test(q)) return true;
...
function sameTopic(a: any, b: any) {
  ...
  const paired = [
    ...
    [/(relatorio|previa|resultado|investid|metrica)/,/(relatorio|previa|resultado|investid|metrica|lead|cpl|campanh)/],
    [/(crm|dora|iara|ia|automacao|integracao)/,/(crm|dora|iara|ia|automacao|integracao|lead)/],
    [/(cobrar|cobranca|pagamento|saldo|cartao)/,/(cobrad|cobrar|pagamento|saldo|cartao|meta)/],
    [/(vejam|resultado|desempenho)/,/(verific|resultado|desempenho|campanh|lead)/],
  ];
...
function classifyClient(c: any, asOf: Date) {
  ...
  const clientActionable = msgs.filter((m: any) => !m.is_team && isActionableClient(m));
  ...
  if (latestReq) {
    ...
    if (closure) {
      ...
      return { kind: "NAO_COBRAR", reason: `O cliente pediu algo em ${localDateTime(latestReq.event_at)} e depois houve execução/entrega compatível em ${localDateTime(closure.event_at)}${approval ? `, seguida de confirmação positiva do cliente em ${localDateTime(approval.event_at)}` : ""}.`, confidence: "ALTA" };
    }
    ...
    if (dependency) {
      ...
      if (!responseAfterDependency) return { kind: "NAO_COBRAR", reason: `Existe dependência pedida ao cliente em ${localDateTime(dependency.event_at)} e não foi localizada resposta posterior dele dentro do corte.`, confidence: "ALTA" };
    }
    ...
    if (!teamLater.length) {
      ...
      return { kind: level, priority: severe ? "CRITICAL" : "HIGH", owner_area: area, owner_person: owner, context: `Em ${localDateTime(latestReq.event_at)}, o cliente escreveu: “${msgText(latestReq)}”. Até ${localDateTime(asOf)}, não foi localizada mensagem posterior da equipe no grupo.`, situation: severe ? "Reclamação/pedido relevante do cliente ainda sem retorno posterior localizado." : "Solicitação recente do cliente ainda sem retorno posterior localizado.", charge_action: area === "GT" ? `Cobrar ${owner || "o GT responsável"} por um posicionamento objetivo e próximo passo sobre esse pedido.` : `Cobrar ${owner || "o responsável"} por retorno objetivo e fechamento desse pedido.`, confidence: "ALTA" };
    }
    ...
    if (progress) {
      ...
      return { kind: level, priority: severe ? "CRITICAL" : "HIGH", owner_area: area, owner_person: owner, context: `Cliente: ${localDateTime(latestReq.event_at)} — “${msgText(latestReq)}”. Equipe: ${localDateTime(progress.event_at)} — “${msgText(progress)}”. Não foi localizada confirmação posterior de execução no mesmo assunto${futureDue ? `; há item relacionado com prazo futuro em ${localDateTime(futureDue)}` : ""}.`, situation: futureDue ? "Pedido reconhecido e em andamento dentro de prazo futuro registrado." : "Pedido reconhecido pela equipe, mas ainda sem evidência de fechamento no grupo.", charge_action: level === "COBRAR_AGORA" ? `Cobrar ${owner || "o responsável"} pela execução/retorno final desse pedido agora.` : `Acompanhar ${owner || "o responsável"} e garantir que o ajuste seja concluído e devolvido ao cliente no prazo.`, confidence: "ALTA" };
    }

    const substantive = [...teamLater].reverse().find((m: any) => String(m.body || "").length >= 35 && sameTopic(latestReq, m));
    if (substantive) return { kind: "NAO_COBRAR", reason: `O pedido do cliente em ${localDateTime(latestReq.event_at)} recebeu resposta posterior sobre o mesmo assunto em ${localDateTime(substantive.event_at)}; não há evidência suficiente para afirmar que ficou uma obrigação aberta.`, confidence: "ALTA" };

    const age = minutesBetween(latestReq.event_at, asOf);
    return { kind: age >= 120 ? "COBRAR_AGORA" : "ACOMPANHAR_HOJE", priority: severe ? "CRITICAL" : "HIGH", owner_area: area, owner_person: owner, context: `O cliente fez um pedido/reclamação em ${localDateTime(latestReq.event_at)}, houve atividade posterior da equipe, mas não foi localizada resposta claramente ligada ao mesmo assunto.`, situation: "Há risco de o pedido ter ficado sem fechamento apesar de outras mensagens no grupo.", charge_action: `Cobrar ${owner || "o responsável"} para conferir o assunto e fechar o retorno ao cliente.`, confidence: "MEDIA" };
  }
  ...
  if (overdueCommitment) return { kind: "COBRAR_AGORA", priority: "HIGH", owner_area: "CS", owner_person: overdueCommitment.owner || c.cs_owner || "", context: `Existe compromisso humano confirmado e ainda aberto: “${clip(overdueCommitment.descricao, 280)}”, com prazo em ${localDateTime(overdueCommitment.due_at)}.`, situation: "Compromisso confirmado venceu sem registro de conclusão.", charge_action: `Cobrar ${overdueCommitment.owner || c.cs_owner || "o responsável"} pelo fechamento desse compromisso.`, confidence: "ALTA" };
  ...
  if (manualOverdue) return { kind: "VERIFICAR_INTERNO", priority: "MEDIUM", owner_area: manualOverdue.target_role || "OPERACOES", owner_person: manualOverdue.target_person || "", context: `Há item manual interno vencido: “${clip(manualOverdue.title, 260)}”, prazo ${localDateTime(manualOverdue.due_at)}. Não existe sinal recente suficiente no WhatsApp para tratá-lo automaticamente como dívida ao cliente.`, situation: "Pendência interna a validar antes de cobrar alguém como atraso real perante o cliente.", charge_action: `Verificar com ${manualOverdue.target_person || "o responsável"} se o item continua aberto de fato e atualizar o status/evidência.`, confidence: "MEDIA" };

  const oc = c.onboarding?.case;
  if (oc?.next_action_due && new Date(oc.next_action_due) < asOf && !/cliente/i.test(String(oc.blocked_by || ""))) return { kind: "VERIFICAR_INTERNO", priority: "HIGH", owner_area: "CS", owner_person: c.cs_owner || "", context: `O onboarding registra próxima ação vencida em ${localDateTime(oc.next_action_due)}: “${clip(oc.next_action, 280)}”.`, situation: "Sinal de onboarding vencido; precisa de validação contra a conversa antes de virar cobrança formal.", charge_action: `Verificar com ${c.cs_owner || "o CS responsável"} se a próxima ação ainda está pendente e registrar o fechamento correto.`, confidence: "MEDIA" };
...
function buildAnalysis(ctx: any, asOf: Date) {
  ...
  for (const c of ctx.clients || []) {
    ...
    if (r.kind === "NAO_COBRAR") not_charge.push({ client_id: c.client_id, client_name: c.display_name, reason: r.reason, confidence: r.confidence, candidate_score: c.candidate_score || 0 });
    else items.push({ client_id: c.client_id, client_name: c.display_name, level: r.kind, priority: r.priority || "MEDIUM", owner_area: r.owner_area || "OPERACOES", owner_person: r.owner_person || "", context: r.context, situation: r.situation, charge_action: r.charge_action, confidence: r.confidence || "MEDIA", candidate_score: c.candidate_score || 0 });
  }
  ...
  const red = items.filter((x) => x.level === "COBRAR_AGORA").length, orange = items.filter((x) => x.level === "ACOMPANHAR_HOJE").length;
...
Deno.serve(async (req) => {
  ...
  if (url.searchParams.get("health") === "1") return json({ ok: true, service: "agency-ops-manager-attention-radar", version: 6, engine: "deterministic-temporal-v2", ai_required: false });
  ...
  if (!runId) {
    const { data: inserted, error } = await ops.from("manager_attention_digest_runs").insert({ run_date: runDate, slot, status: "RUNNING", provider: "DETERMINISTIC_V6", fingerprint, context_stats: { candidate_count: Number(ctx?.candidate_count || 0), dry_run: dryRun } }).select("id").single();
    if (error) return json({ ok: false, error: `run_insert:${error.message}` }, 500);
    runId = inserted.id;
  } else await ops.from("manager_attention_digest_runs").update({ status: "RUNNING", provider: "DETERMINISTIC_V6", fingerprint, error: null, skipped_reason: null, context_stats: { candidate_count: Number(ctx?.candidate_count || 0), dry_run: dryRun }, started_at: new Date().toISOString(), finished_at: null }).eq("id", runId);
