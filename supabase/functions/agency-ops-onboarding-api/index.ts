...
Deno.serve(async (req: Request) => {
  ...
  const gtOptions = (walletRows ?? [])
    .filter((row: any) => activeGtNames.has(String(row.gt_owner)))
    .slice(0, 3)
    .map((row: any) => ({ person: row.gt_owner, carteira: row.carteira, ordem: row.ordem }));
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await ops.rpc("assign_onboarding_gt", {
  ...
  if (!canWorkOnboarding) {
    return respond({ worklist: [], assignment_notifications: [], gt_options: [], generated_at: new Date().toISOString() });
  }

  let workQuery = ops.from("gt_onboarding_worklist").select("*")
    .order("integration_due_at", { ascending: true, nullsFirst: false })
  ...
  if (canAssign) {
    ...
    assignmentNotifications = (requests ?? []).map((request: any) => {
      ...
      return {
        ...
        type: "ONBOARDING_GT_ASSIGNMENT_REQUIRED",
        level: "HIGH",
        title: "Selecionar gestor de tráfego",
        description: confirmed
          ? `A 1ª reunião de apresentação de ${clientName} foi concluída. Selecione o GT para liberar a integração e a carteira.`
          : `${clientName} já avançou além da 1ª apresentação. Selecione o GT para liberar a integração e a carteira.`,
        ...
        metadata: {
          ...
          onboarding_case_id: request.case_id,
          action_type: "ASSIGN_GT",
          action_status: "PENDING",
          gt_options: gtOptions,
        },
      };
    });
  }
  ...
  return respond({
    worklist: workResult.data ?? [],
    assignment_notifications: assignmentNotifications,
    gt_options: canAssign ? gtOptions : [],
    profile: { person, role, can_assign_gt: canAssign },
