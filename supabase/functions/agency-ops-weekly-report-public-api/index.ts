...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const {data:report,error}=await ops.from("weekly_client_reports").select("client_name,week_start,week_end,status,snapshot,generated_at").eq("public_token",token).eq("status","READY").maybeSingle();
    ...
    const paths=[...new Set(creatives.map((r:Row)=>String(r.preview_storage_path||"")).filter(Boolean))];
    if(paths.length){
      ...
      if(!signError&&Array.isArray(data)){
        ...
        data.forEach((item:any,index:number)=>{const path=String(item?.path||paths[index]||"");const url=String(item?.signedUrl||"");if(path&&url)map.set(path,url);});
        snapshot.creatives=creatives.map((r:Row)=>({...r,image_url:map.get(String(r.preview_storage_path||""))||null}));
      }
    }
    return json({ok:true,client_name:report.client_name,week_start:report.week_start,week_end:report.week_end,generated_at:report.generated_at,report:snapshot});
