...
async function ensureDays(ops:any) {
  ...
  if (![1,2,3,4,5].includes(dow)) return [];
  const period=reportPeriodForCollectionDay(today,dow), reportDate=period.end, nowIso=new Date().toISOString();
...
async function ensureClientDay(ops:any, slug:string) {
  const today=localDate(), dow=localWeekday();
  if(![1,2,3,4,5].includes(dow)) return {skipped:"weekend"};
  ...
  const period=reportPeriodForCollectionDay(today,dow), reportDate=period.end;
...
async function closeDue(db:any,ops:any,onlySlug?:string) {
  ...
  for(const day of days||[]){
    ...
    let mid:string|null=null, sendError:string|null=null;
    if(c?.group_chat_id){ try{ const sent=await zapiSend(db,c.group_chat_id,consolidatedMessage(c,day,entries||[])); mid=sent.message_id; }catch(e){ sendError=String(e); } }
    await ops.from("commercial_followup_days").update({status:"CLOSED",closed_at:now,consolidated_message_id:mid,source_payload:{...(day.source_payload||{}),summary_send_error:sendError},updated_at:now}).eq("id",day.id);
    await ops.from("commercial_followup_events").insert({followup_client_id:c?.id||null,followup_day_id:day.id,event_type:mid?"GROUP_SUMMARY_SENT":"GROUP_SUMMARY_FAILED",actor:"AUTOMATION",message_id:mid,payload:{error:sendError}});
    out.push({day_id:day.id,closed:true,message_id:mid,error:sendError});
  }
  return out;
}
...
async function runTick(db:any,ops:any) {
  ...
  const reminders=await sendReminders(db,ops);
  const closed=await closeDue(db,ops);
  const weekly=await sendWeekly(db,ops);
...
Deno.serve(async(req)=>{
  ...
  if(action==="create_client"){
    ...
    const payload={client_id:body.client_id||null,slug,display_name,collection_mode:body.collection_mode||"GRUPO_MANUAL",automation_enabled:false,group_chat_id:body.group_chat_id||null,group_chat_name:body.group_chat_name||null,connected_phone:"5513988051839",checkin_time:body.checkin_time||"08:15",questions:Array.isArray(body.questions)?body.questions:Object.keys(METRIC_LABELS),source_config:{created_from_dashboard:true}};
