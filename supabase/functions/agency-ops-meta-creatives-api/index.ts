...
function exact(actions: Action[] | undefined, type: string) {
  const row = (actions || []).find(
    (item) => item.action_type.toLowerCase() === type.toLowerCase(),
  );
  if (!row) return null;
...
function contains(actions: Action[] | undefined, needle: string) {
  ...
  for (const row of (actions || []).filter((item) =>
    item.action_type.toLowerCase().includes(needle.toLowerCase())
  )) {
    const value = Number(row.value);
    if (Number.isFinite(value) && (!best || value > best.value)) {
      best = { value, source: row.action_type };
    }
  }
  return best;
}
...
function canonical(adName: string | null, actions: Action[] | undefined) {
  const lead = exact(actions, "lead") ??
    exact(actions, "onsite_conversion.lead_grouped") ??
    exact(actions, "offsite_complete_registration_add_meta_leads");
  const message = contains(actions, "messaging_conversation_started") ??
    contains(actions, "total_messaging_connection");
  const messageByName = /(^|[^a-z])(wpp|whats|whatsapp|mensagem)([^a-z]|$)/i.test(
    String(adName || "").toLowerCase(),
  );
  ...
  for (let offset = 0; offset < paths.length; offset += 50) {
    ...
    const body = new URLSearchParams({
      ...
      batch: JSON.stringify(
        chunk.map((relativeUrl) => ({ method: "GET", relative_url: relativeUrl })),
      ),
    });
    const response = await fetch(GRAPH_ROOT, {
      method: "POST",
    ...
    if (!response.ok || !Array.isArray(batch)) {
      throw new Error(String(batch?.error?.message || `Meta batch HTTP ${response.status}`));
    }
    batch.forEach((item: any, index: number) => {
      ...
      try {
        ...
        // A resposta isolada inválida não deve derrubar os outros anúncios do lote.
      }
    });
  }
  return output;
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "GET" && url.searchParams.get("clients") === "1") {
    let clientsQuery = ops
      .from("clients")
      .select("id,display_name,gt_owner")
      .eq("lifecycle", "ACTIVE")
      .order("display_name", { ascending: true });
    if (walletOnly) clientsQuery = clientsQuery.eq("gt_owner", person);
    const { data: clients, error: clientsError } = await clientsQuery;
    if (clientsError) {
      return respond({ error: "clients_failed", detail: clientsError.message }, 500);
    }
    return respond({
      profile,
      clients: (clients || []).map((client: Row) => ({
        client_id: client.id,
  ...
  const force = role === "MGMT" && Boolean(body?.force);
  if (!clientId || !WINDOWS.has(periodDays)) return respond({ error: "invalid_parameters" }, 400);

  let clientQuery = ops
    .from("clients")
    .select("id,display_name,lifecycle,gt_owner")
    .eq("id", clientId);
  ...
  const readCache = async () => {
    const [{ data: state }, { data: rows, error }] = await Promise.all([
    ...
    return {
      ...
      state: state || null,
      stale: !isFresh(state?.finished_at),
      creatives: rows || [],
      generated_at: new Date().toISOString(),
    };
  };
  ...
  const { data: currentState } = await ops
    .from("meta_creative_refresh_state")
    .select("status,started_at,finished_at")
    .eq("client_id", clientId)
    .eq("period_days", periodDays)
    .maybeSingle();
  const runningRecently = currentState?.status === "RUNNING" &&
    Date.now() - new Date(currentState.started_at || 0).getTime() < 5 * 60_000;
  ...
  await ops.from("meta_creative_refresh_state").upsert({
    ...
    finished_at: null,
    metadata: { graph_api_version: GRAPH_API_VERSION, requested_by: person },
    updated_at: new Date().toISOString(),
  }, { onConflict: "client_id,period_days" });

  try {
    const { data: integrations, error: integrationError } = await ops
    ...
    const insightGroups = await Promise.all(
      accountIds.map((accountId) => adInsights(accountId, token, dateFrom, dateTo)),
    );
    ...
    }).filter(({ insight, metric, spend }) =>
      spend > 0 || number(insight.impressions) > 0 || metric.results > 0
    ).sort((a, b) => b.spend - a.spend).slice(0, MAX_ADS_PER_CLIENT);
    ...
    const creativeIds = [...new Set(
      [...adById.values()].map((ad) => String(ad.creative?.id || "")).filter(Boolean),
    )];
    const creativeFields = "id,name,thumbnail_url,image_url,effective_object_story_id";
    const creativePaths = creativeIds.map((creativeId) =>
      `${creativeId}?thumbnail_width=600&thumbnail_height=600&fields=${encodeURIComponent(creativeFields)}`
    );
    ...
    const rows = insights.map(({ insight, metric, spend }) => {
      ...
      return {
        ...
        data_status: "OK",
        metadata: { graph_api_version: GRAPH_API_VERSION, source: "META_MARKETING_API_AD_LEVEL" },
        captured_at: capturedAt,
      };
    }).filter((row) => Boolean(row.ad_id));

    if (rows.length) {
      const { error: upsertError } = await ops.from("meta_creative_snapshots").upsert(rows, {
        onConflict: "client_id,period_days,meta_ad_account_id,ad_id",
      });
      if (upsertError) throw upsertError;
    }
    await ops.from("meta_creative_snapshots")
      .delete()
      .eq("client_id", clientId)
      .eq("period_days", periodDays)
