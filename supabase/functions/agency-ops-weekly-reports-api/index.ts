...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const requestedStart=clean(url.searchParams.get("period_start"),10),requestedEnd=clean(url.searchParams.get("period_end"),10),closed=lastClosedWeek();
    const preferredWeekly=periods.find((p:Row)=>p.report_kind==="WEEKLY"&&String(p.period_start)===closed.start&&String(p.period_end)===closed.end)||periods.find((p:Row)=>p.report_kind==="WEEKLY")||periods[0];
    const selectedStart=clean(requestedStart||preferredWeekly?.period_start,10),selectedEnd=clean(requestedEnd||preferredWeekly?.period_end,10);
