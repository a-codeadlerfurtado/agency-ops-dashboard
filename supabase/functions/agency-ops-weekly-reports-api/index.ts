...
function lastClosedWeek(){const now=new Date(),day=now.getUTCDay()||7;const lastSunday=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate()-day));const end=lastSunday.toISOString().slice(0,10);const start=new Date(lastSunday);start.setUTCDate(start.getUTCDate()-6);return{start:start.toISOString().slice(0,10),end};}
  ...
  try{
    if(req.method==="POST"){
      ...
      if(action==="edit_narrative"){
        ...
        const before=report.editorial_narrative||report.snapshot?.narrative||null;
        const{error:historyError}=await ops.from("weekly_report_editorial_history").insert({report_id:reportId,editor:person,before_narrative:before,after_narrative:next});if(historyError)throw historyError;
      ...
      const defaults=lastClosedWeek(),from=clean(body?.date_from||defaults.start,10),to=clean(body?.date_to||defaults.end,10);if(!DATE.test(from)||!DATE.test(to)||from>to)return json({error:"invalid_period"},400);
      let scope=String(body?.scope_type||"ALL").toUpperCase(),scopeValue=clean(body?.scope_value,120);
      if(!["ALL","GT","CLIENT"].includes(scope))return json({error:"invalid_scope"},400);
      if(role==="GT"){
      ...
      return json({ok:true,date_from:from,date_to:to,scope_type:scope,scope_value:scopeValue||null,request_id:data});
    }
    ...
    if(!selectedStart||!selectedEnd)return json({profile,period:null,periods:[],clients,reports:[],summary:{ready:0,review:0,pending:0,error:0,total:0},gt_options:role==="MGMT"?[...new Set(clients.map((c:Row)=>c.gt_owner).filter(Boolean))]:[person]});
    ...
    const latest=new Map<string,Row>();for(const r of rawReports||[]){const id=String(r.client_id);if(!latest.has(id))latest.set(id,r);}const rows=[...latest.values()].map((r:Row)=>({id:r.id,client_id:r.client_id,client_name:r.client_name,gt_owner:r.gt_owner,period_start:r.period_start,period_end:r.period_end,report_kind:r.report_kind,generation_source:r.generation_source,report_version:r.report_version,status:r.status,audit_status:r.audit_status,attempts:r.attempts,last_error:r.last_error,generated_at:r.generated_at,editorial_status:r.editorial_status,edited_by:r.edited_by,edited_at:r.edited_at,shared_at:r.shared_at,public_path:r.status==="READY"&&r.audit_status==="PASS"?`/relatorio-semanal/${r.public_token}`:null,metrics:r.snapshot?.current||null,narrative:r.editorial_narrative||r.snapshot?.narrative||null,original_narrative:r.snapshot?.narrative||null,creative_count:Array.isArray(r.snapshot?.creatives)?r.snapshot.creatives.length:0,audit:r.snapshot?.audit||null,data_locked:true}));
