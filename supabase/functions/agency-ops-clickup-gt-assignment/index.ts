...
Deno.serve(async (req) => {
  ...
  if (!routingSecret || req.headers.get("x-clickup-gt-routing-key") !== routingSecret) {
  ...
  const { data: task, error: taskError } = await ops.from("generated_tasks")
    .select("id, client_id, clickup_task_id, area, status")
    .eq("id", generatedTaskId)
    .maybeSingle();
  if (taskError || !task) return json({ ok: false, error: taskError?.message || "task_not_found" }, 404);

  if (task.status !== "ENVIADA" || !task.clickup_task_id) {
    return json({ ok: true, skipped: true, reason: "task_not_sent" });
  }

  const area = String(task.area || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
  if (!area.includes("gestor de trafego") && !area.includes("trafego pago")) {
    return json({ ok: true, skipped: true, reason: "not_gt_task" });
  }
  ...
  if (!gtOwner) {
    await ops.from("task_engine_gt_assignment_audit").insert({
      generated_task_id: task.id,
      clickup_task_id: task.clickup_task_id,
      client_id: task.client_id,
      status: "SKIPPED",
      detail: "client_without_gt_owner",
    });
    return json({ ok: true, skipped: true, reason: "client_without_gt_owner" });
  }

  const { data: mapping } = await ops.from("task_engine_gt_clickup_map")
    .select("gt_owner, clickup_user_id")
    .eq("gt_owner", gtOwner)
    .eq("active", true)
    .maybeSingle();

  if (!mapping?.clickup_user_id) {
    await ops.from("task_engine_gt_assignment_audit").insert({
      generated_task_id: task.id,
      clickup_task_id: task.clickup_task_id,
      client_id: task.client_id,
      gt_owner: gtOwner,
      status: "SKIPPED",
      detail: "gt_without_clickup_mapping",
    });
    return json({ ok: true, skipped: true, reason: "gt_without_clickup_mapping", gt_owner: gtOwner });
  }
  ...
  const response = await fetch(`https://api.clickup.com/api/v2/task/${task.clickup_task_id}`, {
    method: "PUT",
    headers: { Authorization: clickupToken, "content-type": "application/json" },
    body: JSON.stringify({ assignees: { add: [targetId], rem: removeIds } }),
  });
  ...
  await ops.from("task_engine_gt_assignment_audit").insert({
    ...
    client_id: task.client_id,
    gt_owner: gtOwner,
    clickup_user_id: targetId,
    status: response.ok ? "APPLIED" : "FAILED",
    detail: response.ok ? `assigned_to:${gtOwner}` : `clickup_${response.status}:${raw.slice(0, 350)}`,
  });
