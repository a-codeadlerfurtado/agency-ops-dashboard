...
Deno.serve(async(req:Request)=>{
  ...
  const db=createClient(supabaseUrl,serviceRole,{auth:{persistSession:false,autoRefreshToken:false}}),ops=db.schema("agency_ops"),userKey=authData.user.id;
  ...
  if(url.searchParams.get("probe")==="1")return respond({ok:true,profile:{person,adler_only:true}});
  let clientId=String(url.searchParams.get("client_id")||"").trim();
  ...
  if(!clientId&&clientName){
    const {data:matches,error}=await ops.from("clients").select("id,display_name").eq("display_name",clientName).limit(2);
    if(error)return respond({error:"client_lookup_failed",detail:error.message},500);
  ...
  const allRuns=runs||[],selectedRun=requestedRun?allRuns.find((row:Row)=>String(row.id)===requestedRun):allRuns[0],selectedRunId=selectedRun?String(selectedRun.id):"";
  ...
  if(exportMode==="summary"){
    ...
    if(clientId)q=q.eq("client_id",clientId);const {data,error}=await q;if(error)return respond({error:"export_failed",detail:error.message},500);
    ...
    return csvResponse([header,...(data||[]).map((x:Row)=>[x.client_name,x.gt_owner,x.snapshot_date,x.period_days,x.date_from,x.date_to,x.requested_period_days,x.available_period_days,x.is_partial_period,x.spend,x.impressions,x.reach,x.clicks,x.ctr,x.cpc,x.cpm,x.frequency,x.leads,x.results,x.cpl,x.cost_per_result,x.campaign_count,x.active_campaigns,x.paused_campaigns,x.data_status])],clientId?`meta-${selectedRun.snapshot_date}-cliente.csv`:`meta-${selectedRun.snapshot_date}-geral.csv`);
  }
  if(exportMode==="detailed"){
    ...
    return csvResponse([header,...(data||[]).map((x:Row)=>[x.client_name,x.gt_owner,x.snapshot_date,x.period_days,x.date_from,x.date_to,x.meta_ad_account_id,x.campaign_id,x.campaign_name,x.campaign_status,x.objective,x.spend,x.impressions,x.reach,x.clicks,x.ctr,x.cpc,x.cpm,x.frequency,x.leads,x.results,x.cpl,x.cost_per_result,x.result_type,x.data_status])],clientId?`meta-${selectedRun.snapshot_date}-cliente-detalhado.csv`:`meta-${selectedRun.snapshot_date}-geral-detalhado.csv`);
  }

  if(clientId){
    const [{data:client},{data:snapshots,error:snapError}]=await Promise.all([
      ops.from("clients").select("id,display_name,lifecycle,entrada,saida,gt_owner,cs_owner").eq("id",clientId).maybeSingle(),
    ...
    const runIdForCampaigns=requestedRun||String((snapshots||[])[0]?.run_id||"");let campaigns:Row[]=[];
    if(runIdForCampaigns){const result=await ops.from("meta_campaign_performance_snapshots").select("*").eq("client_id",clientId).eq("run_id",runIdForCampaigns).order("period_days",{ascending:true}).order("spend",{ascending:false}).limit(1500);if(!result.error)campaigns=result.data||[];}
  ...
  if(selectedRunId){
    ...
    if(result.error)return respond({error:"snapshots_failed",detail:result.error.message},500);snapshots=result.data||[];
    const idx=allRuns.findIndex((row:Row)=>String(row.id)===selectedRunId),previousRun=idx>=0?allRuns[idx+1]:null;
    if(previousRun){const prev=await ops.from("meta_performance_snapshots").select("client_id,period_days,spend,impressions,reach,clicks,ctr,cpc,cpm,frequency,leads,results,cpl,cost_per_result,data_status").eq("run_id",previousRun.id).order("client_name",{ascending:true}).limit(1000);previousSnapshots=prev.data||[];}
  }
  ...
  const activeSet=new Set((activeIds||[]).map((r:Row)=>String(r.id))),integratedSet=new Set((integrations||[]).filter((r:Row)=>activeSet.has(String(r.client_id))).map((r:Row)=>String(r.client_id))),status7=snapshots.filter((r:Row)=>Number(r.period_days)===7);
  const coverage={active_clients:Number(activeCount||0),integrated_clients:integratedSet.size,without_meta:Math.max(0,Number(activeCount||0)-integratedSet.size),ok:status7.filter((r:Row)=>r.data_status==="OK").length,partial_period:status7.filter((r:Row)=>r.data_status==="PARTIAL_PERIOD").length,no_campaigns:status7.filter((r:Row)=>r.data_status==="NO_CAMPAIGNS").length,no_delivery:status7.filter((r:Row)=>r.data_status==="NO_DELIVERY").length,api_issues:status7.filter((r:Row)=>["API_ERROR","API_PARTIAL"].includes(String(r.data_status))).length};
