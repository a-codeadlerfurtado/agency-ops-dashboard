-- Busca continua varrendo o corpus inteiro do cliente, mas o payload enviado ao
-- modelo fica focado: trechos relacionados ao pedido + trechos de configuração
-- (CRM/Make/budget/leads) + uma pequena janela recente. Isso preserva profundidade
-- operacional sem estourar o tempo do backend de IA.
...
event_msgs as (
  ...
  where wm.chat_id = p_chat_id
    ...
    and coalesce(wm.event_at, wm.received_at)
        between p_event_created_at - interval '15 minutes' and p_event_created_at + interval '15 minutes'
  order by coalesce(wm.event_at, wm.received_at) asc
  limit 20
),
recent_msgs as (
  select wm.id, wm.message_id, wm.chat_id, wm.chat_name, wm.sender_name, wm.sender_phone,
         wm.from_me, left(coalesce(nullif(wm.text_body,''), nullif(wm.caption,'')), 1600) as body,
         coalesce(wm.event_at, wm.received_at) as occurred_at
  from agency_ops.whatsapp_messages wm
...
relevant_msgs as (
  ...
  from (
    select wm.id, wm.message_id, wm.chat_id, wm.chat_name, wm.sender_name, wm.sender_phone,
           wm.from_me, left(coalesce(nullif(wm.text_body,''), nullif(wm.caption,'')), 2000) as body,
           coalesce(wm.event_at, wm.received_at) as occurred_at,
...
config_msgs as (
  select wm.id, wm.message_id, wm.chat_id, wm.chat_name, wm.sender_name, wm.sender_phone,
         wm.from_me, left(coalesce(nullif(wm.text_body,''), nullif(wm.caption,'')), 1800) as body,
         coalesce(wm.event_at, wm.received_at) as occurred_at
  from agency_ops.whatsapp_messages wm
  join chats c on c.chat_id = wm.chat_id
  where coalesce(nullif(wm.text_body,''), nullif(wm.caption,'')) is not null
...
briefing as (
  select n.notion_page_id, n.title, n.page_url, n.notion_last_edited_at, n.last_fetched_at,
         n.extracted_profile,
         left(coalesce(n.content_markdown,''), 2000) as content_markdown
...
select jsonb_strip_nulls(jsonb_build_object(
  ...
  'historico_search', jsonb_build_object(
    ...
    'strategy', 'busca_no_corpus_inteiro:8_relacionadas+12_configuracao+8_recentes',
    'query_excerpt', left(coalesce(p_excerpt,''), 1600),
