-- Radar de Anuncios: keep pilot collection independent from broad scheduled refresh.
-- External collection may be enabled for controlled pilot runs while recurring scans remain off.

alter table agency_ops.ad_radar_runtime_config
  add column if not exists scheduled_refresh_enabled boolean not null default false;

update agency_ops.ad_radar_runtime_config
   set scheduled_refresh_enabled = false,
       updated_at = now()
 where id = 1
   and scheduled_refresh_enabled is distinct from false;
...
begin
  ...
  if not found
     or not cfg.enabled
     or not cfg.external_collection_enabled
     or not cfg.scheduled_refresh_enabled
  then
    return 0;
  end if;
