-- Task Engine v10: contexto operacional rico, metadados estruturados e busca no histórico inteiro.
...
alter table agency_ops.generated_tasks
  ...
  add column if not exists requester_message_id text,
  add column if not exists operational_context jsonb,
  add column if not exists context_conflicts jsonb,
  add column if not exists context_sources jsonb,
  add column if not exists completion_criteria jsonb;

create index if not exists generated_tasks_category_created_idx
  on agency_ops.generated_tasks (task_category, created_at desc);
create index if not exists generated_tasks_requester_created_idx
  on agency_ops.generated_tasks (requester_name, created_at desc);
...
raw_terms as (
  ...
  from regexp_split_to_table(
    lower(extensions.unaccent(coalesce(p_excerpt,''))),
    '[^a-z0-9]+'
  ) w
  where length(w) >= 4
...
briefing as (
  select n.notion_page_id, n.title, n.page_url, n.notion_last_edited_at, n.last_fetched_at,
         n.extracted_profile,
         left(coalesce(n.content_markdown,''), 24000) as content_markdown
...
select jsonb_strip_nulls(jsonb_build_object(
  ...
  'historico_search', jsonb_build_object(
    ...
    'strategy', 'full_text_or_em_todo_o_corpus_com_recuperacao_dos_50_trechos_mais_relevantes',
