...
select c.id client_id,c.display_name,c.lifecycle,c.service,c.cs_owner,c.gt_owner,
       b.account_key,b.balance,b.available_balance,b.spend_cap,b.amount_spent,b.currency,
       b.funding_type,b.funding_type_label,b.payment_display,b.balance_source,b.account_status,b.disable_reason,b.checked_at,
       i.latest_spend_date,i.latest_insight_checked_at,coalesce(i.spend_7d,0)::numeric spend_7d,
       case when coalesce(i.spend_7d,0)>0 then round(i.spend_7d/greatest(1,least(7,coalesce(current_date-c.entrada+1,7)))::numeric,2) else 0::numeric end avg_daily_spend,
       ...
       case when b.funding_type=20 and coalesce(i.spend_7d,0)>0 then round(b.available_balance/(i.spend_7d/greatest(1,least(7,coalesce(current_date-c.entrada+1,7)))::numeric),1) else null::numeric end days_remaining,
       greatest(1,least(7,coalesce(current_date-c.entrada+1,7))) spend_window_days,
...
left join lateral (
  ...
  where ci.client_id=c.id and ci.system='META_BM' and ci.meta_ad_account_id is not null
    and (
      ci.external_id=b.account_key or ci.external_name=b.account_key
      or ((select count(*) from agency_ops.client_integrations ci2 where ci2.client_id=c.id and ci2.system='META_BM' and ci2.meta_ad_account_id is not null)=1)
    )
