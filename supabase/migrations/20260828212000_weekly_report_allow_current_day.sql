-- Allow ad-hoc client reports to include the current day.
-- Future dates remain blocked. A report ending today is a partial-day snapshot
-- and must be presented as such in the client-facing UI.
...
begin
  ...
  if p_date_to > v_today then
    raise exception 'report period cannot end in the future';
  end if;
  if p_date_from < p_date_to - 89 then
    raise exception 'report period cannot exceed 90 days';
  end if;
  ...
  select net.http_post(
    ...
    body := jsonb_build_object(
      ...
      'partial_current_day',(p_date_to = v_today)
    ),
