...
internal sealed class MeetLocalBridgeV2 : IDisposable
{
    private readonly MeetDesktopCaptureV2 capture;
    ...
    public MeetLocalBridgeV2(MeetDesktopCaptureV2 capture) => this.capture = capture;
    ...
    private async Task HandleAsync(TcpClient client)
    {
        ...
        using (var stream = client.GetStream())
        {
            try
            {
                ...
                {
                    var rows = capture.GetLive(ParseQueryLong(request.Path, "after"));
    ...
    {
        var buffer = new byte[8192]; using var ms = new MemoryStream(); int headerEnd = -1;
        while (headerEnd < 0 && ms.Length < 65536)
        {
            var read = await stream.ReadAsync(buffer, ct); if (read <= 0) return null;
            ms.Write(buffer, 0, read); headerEnd = FindHeaderEnd(ms.GetBuffer(), (int)ms.Length);
        }
        if (headerEnd < 0) return null;
        var all = ms.ToArray(); var headerText = Encoding.ASCII.GetString(all, 0, headerEnd);
        var lines = headerText.Split("\r\n", StringSplitOptions.None); var first = lines.FirstOrDefault()?.Split(' ', 3);
        ...
        var body = contentLength > 0 && all.Length >= bodyStart
    ...
    {
        var bodyBytes = Encoding.UTF8.GetBytes(JsonSerializer.Serialize(payload, json));
        ...
        await stream.WriteAsync(Encoding.ASCII.GetBytes(headers), cts.Token);
