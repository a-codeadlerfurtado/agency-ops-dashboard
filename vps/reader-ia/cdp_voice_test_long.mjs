...
await cmd('Runtime.enable');await cmd('DOM.enable');
await sleep(3000);
...
for(let i=0;i<36;i++){
  await sleep(1000);
  const r=await cmd('Runtime.evaluate',{returnByValue:true,expression:`({status:document.getElementById('status')?.textContent,sentences:document.querySelectorAll('.sentence').length})`});
  const v=r.result.value;if(v?.sentences>0){ready=true;break;}
}
if(!ready) throw new Error('translation never became ready');
