...
async function beginSession() {
  ...
  const seededId = norm(status?.state?.desktop_session_id);
  const seededStartedAt = norm(status?.state?.started_at);
  const seededStartedMs = Date.parse(seededStartedAt);
  const started = Number.isFinite(seededStartedMs) ? seededStartedMs : Date.now();
  session = {
    id: seededId || `${meetingCode}-${started}-${crypto.randomUUID()}`,
    meeting_code: meetingCode, meeting_url: location.href, title: getMeetingTitle(),
    started_at: Number.isFinite(seededStartedMs) ? seededStartedAt : new Date(started).toISOString(), started_epoch: started, path: location.pathname,
    capture_mode: "MEET_RTC_AUDIO",
    metadata: { fallback: true, audio_source: "FALLBACK_EXTENSION" },
  };
  warningSent = false;
